...
class FakeCliCommandRunner(CliCommandRunner):
    ...
    def _record_cli(self, command: Sequence[str], current_directory: Path) -> None:
        if self._operation_log is not None:
            self._operation_log.append(FakeCli(command=tuple(command), current_directory=current_directory))
