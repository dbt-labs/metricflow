...
def test_profile_performance_using_json_manifest(
    ...
    mf_profile_explain_saved_queries(
        semantic_manifest, sql_client, saved_query_count=100, use_semantic_graph=False, profile=True
    )
...
def mf_load_manifest_from_json_file(json_file_path: Path) -> SemanticManifest:
    ...
    This uses a dummy project configuration as that's outside of `PydanticSemanticManifest`.
    """
