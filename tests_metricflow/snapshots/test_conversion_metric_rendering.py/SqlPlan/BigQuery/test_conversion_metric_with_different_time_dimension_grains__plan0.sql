...
SELECT
  CAST(nr_subq_11.buys_month AS FLOAT64) / CAST(NULLIF(nr_subq_11.visits, 0) AS FLOAT64) AS visit_buy_conversion_rate_with_monthly_conversion
FROM (
          ...
          ON
            (
              ...
              (
                ...
                nr_subq_4.metric_time__month > DATE_SUB(CAST(nr_subq_6.metric_time__month AS DATETIME), INTERVAL 1 month)
              )
            )
        ) nr_subq_7
      ) nr_subq_8
