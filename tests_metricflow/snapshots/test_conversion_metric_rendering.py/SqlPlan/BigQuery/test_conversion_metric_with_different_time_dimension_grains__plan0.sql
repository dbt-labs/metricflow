...
-- Write to DataTable
SELECT
  subq_14.visit_buy_conversion_rate_with_monthly_conversion
FROM (
  -- Compute Metrics via Expressions
  SELECT
    CAST(subq_13.__buys_month AS FLOAT64) / CAST(NULLIF(subq_13.__visits, 0) AS FLOAT64) AS visit_buy_conversion_rate_with_monthly_conversion
  FROM (
    -- Combine Aggregated Outputs
    SELECT
      MAX(subq_3.__visits) AS __visits
      , MAX(subq_12.__buys_month) AS __buys_month
    FROM (
    ...
    CROSS JOIN (
      -- Aggregate Inputs for Simple Metrics
      SELECT
        SUM(subq_11.__buys_month) AS __buys_month
      FROM (
        -- Pass Only Elements: ['__buys_month']
        SELECT
          subq_10.__buys_month
        FROM (
          ...
          FROM (
            ...
            SELECT DISTINCT
              ...
              , subq_8.__buys_month AS __buys_month
            FROM (
            ...
            ON
              (
                ...
                (
                  ...
                  subq_5.metric_time__month > DATE_SUB(CAST(subq_8.metric_time__month AS DATETIME), INTERVAL 1 month)
                )
              )
          ) subq_9
        ) subq_10
