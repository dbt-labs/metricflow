...
SELECT
  CAST(MAX(nr_subq_22.buys_month) AS FLOAT64) / CAST(NULLIF(MAX(nr_subq_14.visits), 0) AS FLOAT64) AS visit_buy_conversion_rate_with_monthly_conversion
FROM (
    ...
    ON
      (
        ...
        (
          ...
          nr_subq_16.metric_time__month > DATE_SUB(CAST(nr_subq_18.metric_time__month AS DATETIME), INTERVAL 1 month)
        )
      )
  ) nr_subq_19
) nr_subq_22
