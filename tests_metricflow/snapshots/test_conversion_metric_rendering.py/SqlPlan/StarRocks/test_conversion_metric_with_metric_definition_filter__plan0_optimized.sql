...
SELECT
  ...
  , CAST(__buys AS DOUBLE) / CAST(NULLIF(__visits, 0) AS DOUBLE) AS visit_buy_conversion_rate_with_filter
FROM (
      ...
      ON
        (
          ...
          (
            ...
            subq_29.metric_time__day > DATE_SUB(subq_32.metric_time__day, INTERVAL 7 DAY)
          )
        )
    ) subq_33
    GROUP BY
