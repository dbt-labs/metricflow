...
SELECT
  CAST(MAX(subq_27.__buys_month) AS DOUBLE PRECISION) / CAST(NULLIF(MAX(subq_18.__visits), 0) AS DOUBLE PRECISION) AS visit_buy_conversion_rate_with_monthly_conversion
FROM (
...
CROSS JOIN (
  ...
  FROM (
    ...
    SELECT DISTINCT
      ...
      , subq_23.__buys_month AS __buys_month
    FROM sma_28019_cte
    INNER JOIN (
