test_name: test_conversion_metric_with_base_measure_filter
...
SELECT
  ...
  , subq_19.visit_buy_conversion_rate_with_base_measure_filter
FROM (
  -- Compute Metrics via Expressions
  SELECT
    subq_18.metric_time__day
    , CAST(subq_18.__buys AS DOUBLE PRECISION) / CAST(NULLIF(subq_18.__visits, 0) AS DOUBLE PRECISION) AS visit_buy_conversion_rate_with_base_measure_filter
  FROM (
