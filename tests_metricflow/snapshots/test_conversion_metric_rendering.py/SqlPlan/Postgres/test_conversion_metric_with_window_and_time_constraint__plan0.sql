...
SELECT
  ...
  , CAST(nr_subq_15.buys AS DOUBLE PRECISION) / CAST(NULLIF(nr_subq_15.visits, 0) AS DOUBLE PRECISION) AS visit_buy_conversion_rate_7days
FROM (
  -- Combine Aggregated Outputs
  SELECT
    COALESCE(nr_subq_5.metric_time__day, nr_subq_14.metric_time__day) AS metric_time__day
          ...
          ON
            (
              ...
              (
                ...
                nr_subq_8.metric_time__day > nr_subq_10.metric_time__day - MAKE_INTERVAL(days => 7)
              )
            )
        ) nr_subq_11
      ) nr_subq_12
  ...
  ON
    (
      ...
      nr_subq_5.metric_time__day = nr_subq_14.metric_time__day
    )
  GROUP BY
    COALESCE(nr_subq_5.metric_time__day, nr_subq_14.metric_time__day)
