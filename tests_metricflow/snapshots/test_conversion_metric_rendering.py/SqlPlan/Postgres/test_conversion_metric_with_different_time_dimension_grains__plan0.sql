...
SELECT
  CAST(nr_subq_11.buys_month AS DOUBLE PRECISION) / CAST(NULLIF(nr_subq_11.visits, 0) AS DOUBLE PRECISION) AS visit_buy_conversion_rate_with_monthly_conversion
FROM (
          ...
          ON
            (
              ...
              (
                ...
                nr_subq_4.metric_time__month > nr_subq_6.metric_time__month - MAKE_INTERVAL(months => 1)
              )
            )
        ) nr_subq_7
      ) nr_subq_8
