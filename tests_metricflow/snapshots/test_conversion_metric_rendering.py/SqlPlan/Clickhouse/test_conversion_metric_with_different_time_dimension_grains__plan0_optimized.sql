...
SELECT
  CAST(MAX(subq_27.buys_month) AS Nullable(DOUBLE PRECISION)) / CAST(NULLIF(MAX(subq_18.visits), 0) AS Nullable(DOUBLE PRECISION)) AS visit_buy_conversion_rate_with_monthly_conversion
FROM (
