...
FROM (
  SELECT
    CAST(subq_16.__buys_month AS Nullable(Float64)) / CAST(NULLIF(subq_16.__visits, 0) AS Nullable(Float64)) AS visit_buy_conversion_rate_with_monthly_conversion
  FROM (
    SELECT
              ...
              ON
                (
                  ...
                  (
                    ...
                    subq_7.metric_time__month > addMonths(subq_10.metric_time__month, -1)
                  )
                )
            ) subq_11
          ) subq_12
