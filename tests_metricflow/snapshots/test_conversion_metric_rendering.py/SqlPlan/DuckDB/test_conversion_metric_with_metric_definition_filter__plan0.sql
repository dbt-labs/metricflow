...
SELECT
  ...
  , subq_17.visit_buy_conversion_rate_with_filter
FROM (
  -- Compute Metrics via Expressions
  SELECT
    subq_16.metric_time__day
    , CAST(subq_16.__buys AS DOUBLE) / CAST(NULLIF(subq_16.__visits, 0) AS DOUBLE) AS visit_buy_conversion_rate_with_filter
  FROM (
