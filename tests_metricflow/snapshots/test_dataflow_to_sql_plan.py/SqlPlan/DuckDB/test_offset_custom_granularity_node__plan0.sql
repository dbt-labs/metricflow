...
SELECT
  ...
  , subq_0.ds__martian_day__lead AS metric_time__martian_day
FROM cte_0 cte_0
INNER JOIN (
  -- Offset Custom Granularity
  SELECT
    cte_0.ds__martian_day
    , LEAD(cte_0.ds__martian_day, 3) OVER (ORDER BY cte_0.ds__martian_day) AS ds__martian_day__lead
  FROM cte_0 cte_0
  GROUP BY
    cte_0.ds__martian_day
) subq_0
ON
  cte_0.ds__martian_day = subq_0.ds__martian_day
