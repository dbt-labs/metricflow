...
SELECT
  ...
  , subq_13.homonymous_metric_and_entity
  , subq_13.homonymous_metric_and_dimension
FROM (
  -- Combine Aggregated Outputs
  SELECT
    COALESCE(subq_6.metric_time__day, subq_12.metric_time__day) AS metric_time__day
    , MAX(subq_6.homonymous_metric_and_entity) AS homonymous_metric_and_entity
    , MAX(subq_12.homonymous_metric_and_dimension) AS homonymous_metric_and_dimension
  FROM (
  ...
  FULL OUTER JOIN (
    ...
    SELECT
      ...
      , subq_11.__homonymous_metric_and_dimension AS homonymous_metric_and_dimension
    FROM (
      -- Aggregate Inputs for Simple Metrics
      SELECT
        subq_10.metric_time__day
        , SUM(subq_10.__homonymous_metric_and_dimension) AS __homonymous_metric_and_dimension
      FROM (
        -- Select: ['__homonymous_metric_and_dimension', 'metric_time__day']
        SELECT
          subq_9.metric_time__day
          , subq_9.__homonymous_metric_and_dimension
        FROM (
          -- Constrain Output with WHERE
          SELECT
            subq_8.homonymous_metric_and_dimension AS __homonymous_metric_and_dimension
            , subq_8.metric_time__day
          FROM (
            -- Select: ['__homonymous_metric_and_dimension', 'metric_time__day']
            SELECT
              subq_7.metric_time__day
              , subq_7.__homonymous_metric_and_dimension AS homonymous_metric_and_dimension
            FROM (
              -- Metric Time Dimension 'ds'
              SELECT
                subq_0.ds__day
