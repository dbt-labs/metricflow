...
SELECT
  CAST(MAX(subq_38.buys) AS FLOAT64) / CAST(NULLIF(MAX(subq_26.visits), 0) AS FLOAT64) AS visit_buy_conversion_rate_7days
FROM (
    ...
    ON
      (
        ...
        (
          ...
          subq_31.metric_time__day > DATE_SUB(CAST(subq_34.metric_time__day AS DATETIME), INTERVAL 7 day)
        )
      )
  ) subq_35
) subq_38
