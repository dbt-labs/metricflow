  ...
  FULL OUTER JOIN (
    ...
    FROM (
      ...
      SELECT DISTINCT
        ...
        , FIRST_VALUE(subq_32.ds__day) OVER (
          PARTITION BY
            subq_35.user
            , subq_35.ds__day
            , subq_35.mf_internal_uuid
      ...
      ON
        (
          ...
          (
            subq_32.ds__day <= subq_35.ds__day
          ) AND (
            subq_32.ds__day > DATE_SUB(CAST(subq_35.ds__day AS DATETIME), INTERVAL 7 day)
          )
        )
    ) subq_36
    GROUP BY
