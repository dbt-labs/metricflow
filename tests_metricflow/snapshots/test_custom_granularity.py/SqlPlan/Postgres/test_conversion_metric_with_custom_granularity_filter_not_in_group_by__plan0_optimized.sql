...
SELECT
  CAST(MAX(nr_subq_30.buys) AS DOUBLE PRECISION) / CAST(NULLIF(MAX(nr_subq_20.visits), 0) AS DOUBLE PRECISION) AS visit_buy_conversion_rate_7days
FROM (
    ...
    ON
      (
        ...
        (
          ...
          nr_subq_24.metric_time__day > nr_subq_26.metric_time__day - MAKE_INTERVAL(days => 7)
        )
      )
  ) nr_subq_27
) nr_subq_30
