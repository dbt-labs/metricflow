...
FROM (
  ...
  FROM (
    ...
    SELECT
      ...
      , CASE
        WHEN subq_27.ds__alien_day__first_value__lead + MAKE_INTERVAL(days => CAST ((cte_6.ds__day__row_number - 1) AS INTEGER)) <= subq_27.ds__alien_day__last_value__lead
          THEN subq_27.ds__alien_day__first_value__lead + MAKE_INTERVAL(days => CAST ((cte_6.ds__day__row_number - 1) AS INTEGER))
        ELSE NULL
