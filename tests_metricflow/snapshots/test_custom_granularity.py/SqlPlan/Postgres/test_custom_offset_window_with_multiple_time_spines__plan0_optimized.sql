...
FROM (
  ...
  FROM (
    ...
    SELECT
      ...
      , CASE
        WHEN subq_28.ts__hour__first_value__lead + MAKE_INTERVAL(hours => CAST ((cte_6.ts__hour__row_number - 1) AS INTEGER)) <= subq_28.ts__hour__last_value__lead
          THEN subq_28.ts__hour__first_value__lead + MAKE_INTERVAL(hours => CAST ((cte_6.ts__hour__row_number - 1) AS INTEGER))
        ELSE NULL
