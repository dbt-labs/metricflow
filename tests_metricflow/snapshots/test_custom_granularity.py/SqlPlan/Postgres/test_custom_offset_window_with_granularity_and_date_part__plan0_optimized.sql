...
FROM (
  ...
  FROM (
    ...
    SELECT
      ...
      , CASE
        WHEN subq_24.ds__martian_day__first_value__offset + MAKE_INTERVAL(days => CAST ((cgb_1_cte.ds__day__row_number - 1) AS INTEGER)) <= subq_24.ds__martian_day__last_value__offset
          THEN subq_24.ds__martian_day__first_value__offset + MAKE_INTERVAL(days => CAST ((cgb_1_cte.ds__day__row_number - 1) AS INTEGER))
