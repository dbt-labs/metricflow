...
FROM (
  ...
  SELECT
    ...
    , DATE_TRUNC('month', nr_subq_24.ds__day__lead) AS booking__ds__month
  ...
  FROM (
    ...
    SELECT
      ...
      , CASE
        WHEN nr_subq_23.ds__martian_day__first_value__lead + MAKE_INTERVAL(days => CAST ((cte_6.ds__day__row_number - 1) AS INTEGER)) <= nr_subq_23.ds__martian_day__last_value__lead
          THEN nr_subq_23.ds__martian_day__first_value__lead + MAKE_INTERVAL(days => CAST ((cte_6.ds__day__row_number - 1) AS INTEGER))
        ELSE NULL
  ...
  GROUP BY
    nr_subq_27.martian_day
    , DATE_TRUNC('month', nr_subq_24.ds__day__lead)
    , EXTRACT(year FROM nr_subq_24.ds__day__lead)
) nr_subq_31
