...
FROM (
  ...
  SELECT
    COALESCE(subq_9.metric_time__martian_day, subq_14.metric_time__martian_day) AS metric_time__martian_day
    , MAX(subq_9.bookings_offset) AS bookings_offset
    , MAX(subq_14.bookings) AS bookings
  FROM (
    -- Compute Metrics via Expressions
    SELECT
      subq_8.metric_time__martian_day
      , subq_8.bookings AS bookings_offset
    FROM (
      -- Aggregate Measures
      SELECT
      ...
      FROM (
        ...
        FROM (
          ...
          FROM (
            -- Pass Only Elements: ['ds__day', 'metric_time__day', 'metric_time__martian_day']
            SELECT
              subq_4.ds__day
  ...
  ON
    subq_9.metric_time__martian_day = subq_14.metric_time__martian_day
  GROUP BY
    COALESCE(subq_9.metric_time__martian_day, subq_14.metric_time__martian_day)
) subq_15
