...
FROM (
  ...
  SELECT
    COALESCE(subq_15.metric_time__week, subq_19.metric_time__week) AS metric_time__week
    , MAX(subq_15.bookings_offset) AS bookings_offset
    , MAX(subq_19.bookings) AS bookings
  FROM (
    -- Compute Metrics via Expressions
    SELECT
      subq_14.metric_time__week
      , subq_14.bookings AS bookings_offset
    FROM (
      -- Aggregate Measures
      SELECT
      ...
      FROM (
        ...
        FROM (
          ...
          FROM (
            -- Pass Only Elements: ['ds__day', 'metric_time__day', 'metric_time__week']
            SELECT
              subq_10.ds__day
            ...
            FROM (
              ...
              SELECT
                ...
                , DATE_TRUNC('week', subq_9.ds__day__lead) AS metric_time__week
              FROM (
                ...
                SELECT
                  ...
                  , CASE
                    WHEN subq_8.ds__day__first_value__lead + INTERVAL (cte_2.ds__day__row_number - 1) day <= subq_8.ds__day__last_value__lead
                      THEN subq_8.ds__day__first_value__lead + INTERVAL (cte_2.ds__day__row_number - 1) day
                    ELSE NULL
  ...
  ON
    subq_15.metric_time__week = subq_19.metric_time__week
  GROUP BY
    COALESCE(subq_15.metric_time__week, subq_19.metric_time__week)
) subq_20
