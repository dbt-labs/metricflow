...
-- Write to DataTable
SELECT
  subq_22.metric_time__alien_day
  , subq_22.booking_fees_since_start_of_month
FROM (
  ...
  FROM (
    ...
    SELECT
      COALESCE(subq_13.metric_time__alien_day, subq_20.metric_time__alien_day) AS metric_time__alien_day
      , MAX(subq_13.booking_fees_start_of_month) AS booking_fees_start_of_month
      , MAX(subq_20.booking_fees) AS booking_fees
    FROM (
      -- Select: ['metric_time__alien_day', 'booking_fees_start_of_month']
      SELECT
        subq_12.metric_time__alien_day
        , subq_12.booking_fees_start_of_month
      FROM (
        -- Join to Time Spine Dataset
        SELECT
          subq_11.metric_time__alien_day AS metric_time__alien_day
          , subq_7.metric_time__day AS metric_time__day
          , subq_7.booking_fees_start_of_month AS booking_fees_start_of_month
        FROM (
          ...
          FROM (
            ...
            FROM (
              ...
              SELECT
                ...
                , subq_8.ds__alien_day AS metric_time__alien_day
              FROM (
        ...
        ON
          DATE_TRUNC('month', subq_11.metric_time__alien_day) = subq_7.metric_time__alien_day
      ) subq_12
    ) subq_13
    FULL OUTER JOIN (
      -- Compute Metrics via Expressions
    ...
    ON
      subq_13.metric_time__alien_day = subq_20.metric_time__alien_day
    GROUP BY
      COALESCE(subq_13.metric_time__alien_day, subq_20.metric_time__alien_day)
  ) subq_21
) subq_22
