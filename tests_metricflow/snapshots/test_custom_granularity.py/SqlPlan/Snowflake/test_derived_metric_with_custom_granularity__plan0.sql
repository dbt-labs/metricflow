...
SELECT
  nr_subq_10.booking__ds__martian_day
  , booking_value * 0.05 / bookers AS booking_fees_per_booker
FROM (
  -- Combine Aggregated Outputs
  SELECT
    COALESCE(nr_subq_4.booking__ds__martian_day, nr_subq_9.booking__ds__martian_day) AS booking__ds__martian_day
    , MAX(nr_subq_4.booking_value) AS booking_value
    , MAX(nr_subq_9.bookers) AS bookers
  FROM (
    -- Compute Metrics via Expressions
    SELECT
      nr_subq_3.booking__ds__martian_day
      , nr_subq_3.booking_value
    FROM (
      -- Aggregate Measures
      SELECT
        nr_subq_2.booking__ds__martian_day
      ...
      FROM (
        ...
        FROM (
          ...
          ON
            nr_subq_28002.booking__ds__day = nr_subq_0.ds
        ) nr_subq_1
      ) nr_subq_2
      GROUP BY
        nr_subq_2.booking__ds__martian_day
  ...
  FULL OUTER JOIN (
    -- Compute Metrics via Expressions
    SELECT
      nr_subq_8.booking__ds__martian_day
      , nr_subq_8.bookers
    FROM (
      -- Aggregate Measures
      SELECT
        nr_subq_7.booking__ds__martian_day
        , COUNT(DISTINCT nr_subq_7.bookers) AS bookers
      FROM (
        -- Pass Only Elements: ['bookers', 'booking__ds__martian_day']
        SELECT
          nr_subq_6.booking__ds__martian_day
          , nr_subq_6.bookers
        FROM (
          -- Metric Time Dimension 'ds'
          -- Join to Custom Granularity Dataset
          SELECT
            ...
            , nr_subq_28002.approximate_continuous_booking_value_p99 AS approximate_continuous_booking_value_p99
            , nr_subq_28002.approximate_discrete_booking_value_p99 AS approximate_discrete_booking_value_p99
            , nr_subq_5.martian_day AS booking__ds__martian_day
          FROM (
            -- Read Elements From Semantic Model 'bookings_source'
            SELECT
              1 AS bookings
          ...
          ON
            nr_subq_28002.booking__ds__day = nr_subq_5.ds
        ) nr_subq_6
      ) nr_subq_7
      GROUP BY
        nr_subq_7.booking__ds__martian_day
  ...
  ON
    nr_subq_4.booking__ds__martian_day = nr_subq_9.booking__ds__martian_day
  GROUP BY
    COALESCE(nr_subq_4.booking__ds__martian_day, nr_subq_9.booking__ds__martian_day)
) nr_subq_10
