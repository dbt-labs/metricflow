...
SELECT
  ...
  , subq_10.subdaily_cumulative_window_metric
FROM (
  -- Compute Metrics via Expressions
  SELECT
    subq_9.metric_time__alien_day
    , subq_9.metric_time__hour
    , subq_9.simple_subdaily_metric_default_day AS subdaily_cumulative_window_metric
  FROM (
    -- Compute Metrics via Expressions
    SELECT
      subq_8.metric_time__alien_day
      , subq_8.metric_time__hour
      , subq_8.__simple_subdaily_metric_default_day AS simple_subdaily_metric_default_day
    FROM (
      -- Aggregate Inputs for Simple Metrics
      SELECT
        subq_7.metric_time__alien_day
        , subq_7.metric_time__hour
        , SUM(subq_7.__simple_subdaily_metric_default_day) AS __simple_subdaily_metric_default_day
      FROM (
        ...
        SELECT
          ...
          , subq_6.__simple_subdaily_metric_default_day
        FROM (
          -- Select: ['__simple_subdaily_metric_default_day', 'metric_time__alien_day', 'metric_time__hour']
          SELECT
          ...
          FROM (
            ...
            FROM (
              ...
              SELECT
                ...
                , TIMESTAMP_TRUNC(subq_3.ts, day) AS metric_time__day
