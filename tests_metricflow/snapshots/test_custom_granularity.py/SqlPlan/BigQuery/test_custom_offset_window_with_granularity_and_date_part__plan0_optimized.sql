...
FROM (
  ...
  FROM (
    ...
    SELECT
      ...
      , CASE
        WHEN DATE_ADD(CAST(subq_28.ds__alien_day__first_value__lead AS DATETIME), INTERVAL cte_6.ds__day__row_number - 1 day) <= subq_28.ds__alien_day__last_value__lead
          THEN DATE_ADD(CAST(subq_28.ds__alien_day__first_value__lead AS DATETIME), INTERVAL cte_6.ds__day__row_number - 1 day)
        ELSE NULL
