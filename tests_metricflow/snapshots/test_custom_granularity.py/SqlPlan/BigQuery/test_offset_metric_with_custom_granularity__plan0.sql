...
FROM (
  ...
  FROM (
    ...
    FROM (
      ...
      SELECT
        nr_subq_5.booking__ds__martian_day
        , nr_subq_5.bookings
      FROM (
        ...
        ON
          DATE_SUB(CAST(nr_subq_3.booking__ds__day AS DATETIME), INTERVAL 5 day) = nr_subq_0.booking__ds__day
        LEFT OUTER JOIN
          ***************************.mf_time_spine nr_subq_4
        ON
          nr_subq_3.booking__ds__day = nr_subq_4.ds
      ) nr_subq_5
    ) nr_subq_6
    GROUP BY
