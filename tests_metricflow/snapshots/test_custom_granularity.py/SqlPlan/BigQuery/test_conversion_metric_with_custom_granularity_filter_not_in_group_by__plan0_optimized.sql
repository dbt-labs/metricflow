...
SELECT
  CAST(MAX(subq_41.__buys) AS FLOAT64) / CAST(NULLIF(MAX(subq_28.__visits), 0) AS FLOAT64) AS visit_buy_conversion_rate_7days
FROM (
    ...
    ON
      (
        ...
        (
          ...
          subq_33.metric_time__day > DATE_SUB(CAST(subq_36.metric_time__day AS DATETIME), INTERVAL 7 day)
        )
      )
  ) subq_37
) subq_41
