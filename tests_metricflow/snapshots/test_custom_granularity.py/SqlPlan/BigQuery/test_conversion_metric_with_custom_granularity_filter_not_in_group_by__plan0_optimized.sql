...
SELECT
  CAST(MAX(nr_subq_30.buys) AS FLOAT64) / CAST(NULLIF(MAX(nr_subq_20.visits), 0) AS FLOAT64) AS visit_buy_conversion_rate_7days
FROM (
    ...
    ON
      (
        ...
        (
          ...
          nr_subq_24.metric_time__day > DATE_SUB(CAST(nr_subq_26.metric_time__day AS DATETIME), INTERVAL 7 day)
        )
      )
  ) nr_subq_27
) nr_subq_30
