...
SELECT
  nr_subq_6.listing__ds__martian_day
  , nr_subq_6.bookings
FROM (
  -- Aggregate Measures
  SELECT
    nr_subq_5.listing__ds__martian_day
    , SUM(nr_subq_5.bookings) AS bookings
  FROM (
    -- Pass Only Elements: ['bookings', 'listing__ds__martian_day']
    SELECT
      nr_subq_4.listing__ds__martian_day
      , nr_subq_4.bookings
    FROM (
      -- Join Standard Outputs
      -- Join to Custom Granularity Dataset
      SELECT
        nr_subq_2.ds__day AS listing__ds__day
      ...
      ON
        nr_subq_2.ds__day = nr_subq_3.ds
    ) nr_subq_4
  ) nr_subq_5
  GROUP BY
    nr_subq_5.listing__ds__martian_day
