...
FROM (
  ...
  FROM (
    ...
    FROM (
        ...
        FROM (
          ...
          FROM (
            ...
            SELECT
              ...
              , CASE WHEN EXTRACT(dow FROM time_spine_src_16006.ds) = 0 THEN EXTRACT(dow FROM time_spine_src_16006.ds) + 7 ELSE EXTRACT(dow FROM time_spine_src_16006.ds) END AS ds__extract_dow
      ...
      ON
        DATEADD(month, -1, subq_6.metric_time__month) = subq_3.metric_time__month
    ) subq_7
  ) subq_8
) subq_9
