...
SELECT
  ...
  , subq_19.booking_fees_last_week_per_booker_this_week
FROM (
  ...
  FROM (
    ...
    SELECT
      COALESCE(subq_11.metric_time__month, subq_17.metric_time__month) AS metric_time__month
      , MAX(subq_11.booking_value) AS booking_value
      , MAX(subq_17.bookers) AS bookers
    FROM (
      -- Compute Metrics via Expressions
      SELECT
        ...
        FROM (
          ...
          FROM (
            ...
            FROM (
              ...
              ON
                DATE_SUB(subq_6.metric_time__day, INTERVAL 1 WEEK) = subq_1.metric_time__day
            ) subq_7
          ) subq_8
        ) subq_9
        GROUP BY
    ...
    ON
      subq_11.metric_time__month = subq_17.metric_time__month
    GROUP BY
      COALESCE(subq_11.metric_time__month, subq_17.metric_time__month)
  ) subq_18
) subq_19
