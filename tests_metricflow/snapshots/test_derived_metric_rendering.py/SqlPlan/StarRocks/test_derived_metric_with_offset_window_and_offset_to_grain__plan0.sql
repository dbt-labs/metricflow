...
FROM (
  ...
  FROM (
    ...
    SELECT
      COALESCE(subq_10.metric_time__day, subq_20.metric_time__day) AS metric_time__day
    ...
    FROM (
      ...
      FROM (
        ...
        ON
          DATE_TRUNC('month', subq_8.metric_time__day) = subq_4.metric_time__day
      ) subq_9
    ) subq_10
    FULL OUTER JOIN (
      -- Compute Metrics via Expressions
      ...
      FROM (
        ...
        ON
          DATE_SUB(subq_18.metric_time__day, INTERVAL 1 MONTH) = subq_14.metric_time__day
      ) subq_19
    ) subq_20
    ON
      subq_10.metric_time__day = subq_20.metric_time__day
    GROUP BY
      COALESCE(subq_10.metric_time__day, subq_20.metric_time__day)
  ) subq_21
) subq_22
