...
FROM (
  ...
  FROM (
    ...
    FROM (
          ...
          FROM (
            ...
            FROM (
              ...
              SELECT
                ...
                , CASE WHEN EXTRACT(DAYOFWEEK FROM time_spine_src_16006.ds) = 1 THEN 7 ELSE EXTRACT(DAYOFWEEK FROM time_spine_src_16006.ds) - 1 END AS ds__extract_dow
                , DAYOFYEAR(time_spine_src_16006.ds) AS ds__extract_doy
      ...
      ON
        DATE_SUB(subq_8.metric_time__month, INTERVAL 1 MONTH) = subq_4.metric_time__month
    ) subq_9
  ) subq_10
) subq_11
