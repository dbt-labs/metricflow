...
FROM (
  ...
  FROM (
    ...
    SELECT
      COALESCE(subq_10.metric_time__day, subq_15.metric_time__day) AS metric_time__day
      , COALESCE(subq_10.metric_time__month, subq_15.metric_time__month) AS metric_time__month
      , COALESCE(subq_10.metric_time__year, subq_15.metric_time__year) AS metric_time__year
    ...
    FROM (
      ...
      FROM (
        ...
        ON
          DATE_SUB(subq_8.metric_time__day, INTERVAL 1 WEEK) = subq_4.metric_time__day
      ) subq_9
    ) subq_10
    FULL OUTER JOIN (
      -- Compute Metrics via Expressions
    ...
    ON
      (
        subq_10.metric_time__day = subq_15.metric_time__day
      ) AND (
        subq_10.metric_time__month = subq_15.metric_time__month
      ) AND (
        subq_10.metric_time__year = subq_15.metric_time__year
      )
    GROUP BY
      COALESCE(subq_10.metric_time__day, subq_15.metric_time__day)
      , COALESCE(subq_10.metric_time__month, subq_15.metric_time__month)
      , COALESCE(subq_10.metric_time__year, subq_15.metric_time__year)
  ) subq_16
) subq_17
