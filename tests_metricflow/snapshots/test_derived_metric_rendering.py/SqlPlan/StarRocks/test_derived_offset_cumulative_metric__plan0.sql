...
SELECT
  ...
  , subq_15.every_2_days_bookers_2_days_ago
FROM (
  ...
  FROM (
    ...
    SELECT
      ...
      , subq_13.bookers AS every_2_days_bookers_2_days_ago
    FROM (
      -- Compute Metrics via Expressions
      SELECT
      ...
      FROM (
        ...
        ON
          DATE_SUB(subq_11.metric_time__day, INTERVAL 2 DAY) = subq_7.metric_time__day
      ) subq_12
    ) subq_13
  ) subq_14
) subq_15
