...
SELECT
  ...
  , subq_14.nested_fill_nulls_without_time_spine
  , subq_14.listings
FROM (
  -- Combine Aggregated Outputs
  SELECT
    COALESCE(subq_7.metric_time__day, subq_13.metric_time__day) AS metric_time__day
    , MAX(subq_7.nested_fill_nulls_without_time_spine) AS nested_fill_nulls_without_time_spine
  ...
  FULL OUTER JOIN (
    ...
    SELECT
      ...
      , subq_12.__listings AS listings
    FROM (
      -- Aggregate Inputs for Simple Metrics
      SELECT
  ...
  ON
    subq_7.metric_time__day = subq_13.metric_time__day
  GROUP BY
    COALESCE(subq_7.metric_time__day, subq_13.metric_time__day)
) subq_14
