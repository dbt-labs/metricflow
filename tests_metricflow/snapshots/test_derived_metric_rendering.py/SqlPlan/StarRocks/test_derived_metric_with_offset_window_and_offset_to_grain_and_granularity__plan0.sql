...
SELECT
  ...
  , subq_22.bookings_month_start_compared_to_1_month_prior
FROM (
  ...
  FROM (
    ...
    SELECT
      COALESCE(subq_10.metric_time__year, subq_20.metric_time__year) AS metric_time__year
      , MAX(subq_10.month_start_bookings) AS month_start_bookings
      , MAX(subq_20.bookings_1_month_ago) AS bookings_1_month_ago
    FROM (
      -- Compute Metrics via Expressions
      SELECT
        subq_9.metric_time__year
        , subq_9.__bookings AS month_start_bookings
      FROM (
        ...
        FROM (
          ...
          FROM (
            ...
            FROM (
              ...
              ON
                DATE_TRUNC('month', subq_5.metric_time__day) = subq_1.metric_time__day
              WHERE subq_5.metric_time__year = subq_5.metric_time__day
            ) subq_6
          ) subq_7
        ) subq_8
        GROUP BY
    ...
    FULL OUTER JOIN (
      ...
      SELECT
        ...
        , subq_19.__bookings AS bookings_1_month_ago
      FROM (
        -- Aggregate Inputs for Simple Metrics
        SELECT
        ...
        FROM (
          ...
          FROM (
            ...
            FROM (
              ...
              ON
                DATE_SUB(subq_15.metric_time__day, INTERVAL 1 MONTH) = subq_11.metric_time__day
            ) subq_16
          ) subq_17
        ) subq_18
        GROUP BY
    ...
    ON
      subq_10.metric_time__year = subq_20.metric_time__year
    GROUP BY
      COALESCE(subq_10.metric_time__year, subq_20.metric_time__year)
  ) subq_21
) subq_22
