...
SELECT
  ...
  , subq_14.derived_shared_alias_1a
  , subq_14.derived_shared_alias_2
FROM (
  -- Order By [] Limit 1
  SELECT
    subq_13.booking__is_instant
    , subq_13.derived_shared_alias_1a
    , subq_13.derived_shared_alias_2
  FROM (
    -- Combine Aggregated Outputs
    SELECT
      COALESCE(subq_6.booking__is_instant, subq_12.booking__is_instant) AS booking__is_instant
      , MAX(subq_6.derived_shared_alias_1a) AS derived_shared_alias_1a
      , MAX(subq_12.derived_shared_alias_2) AS derived_shared_alias_2
    FROM (
      ...
      FROM (
        ...
        SELECT
          ...
          , subq_4.__bookings AS shared_alias
        FROM (
    ...
    FULL OUTER JOIN (
      ...
      FROM (
        ...
        SELECT
          ...
          , subq_10.__instant_bookings AS shared_alias
        FROM (
          -- Aggregate Inputs for Simple Metrics
          SELECT
    ...
    ON
      subq_6.booking__is_instant = subq_12.booking__is_instant
    GROUP BY
      COALESCE(subq_6.booking__is_instant, subq_12.booking__is_instant)
  ) subq_13
  LIMIT 1
) subq_14
