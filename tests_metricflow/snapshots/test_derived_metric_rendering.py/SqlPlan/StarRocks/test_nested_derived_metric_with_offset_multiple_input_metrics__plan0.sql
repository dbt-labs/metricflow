...
SELECT
  ...
  , subq_19.booking_fees_since_start_of_month
FROM (
  ...
  FROM (
    ...
    SELECT
      COALESCE(subq_11.metric_time__day, subq_17.metric_time__day) AS metric_time__day
      , MAX(subq_11.booking_fees_start_of_month) AS booking_fees_start_of_month
      , MAX(subq_17.booking_fees) AS booking_fees
    FROM (
      ...
      ON
        DATE_TRUNC('month', subq_10.metric_time__day) = subq_6.metric_time__day
    ) subq_11
    FULL OUTER JOIN (
      -- Compute Metrics via Expressions
      SELECT
    ...
    ON
      subq_11.metric_time__day = subq_17.metric_time__day
    GROUP BY
      COALESCE(subq_11.metric_time__day, subq_17.metric_time__day)
  ) subq_18
) subq_19
