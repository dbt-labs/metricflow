...
SELECT
  ...
  , subq_13.every_2_days_bookers_2_days_ago
FROM (
  ...
  FROM (
    ...
    SELECT
      ...
      , subq_11.bookers AS every_2_days_bookers_2_days_ago
    FROM (
      -- Compute Metrics via Expressions
      SELECT
      ...
      FROM (
        ...
        INNER JOIN (
          ...
          SELECT
            ...
            , COUNT(DISTINCT subq_5.__bookers) AS __bookers
          FROM (
        ...
        ON
          DATE_SUB(CAST(subq_9.metric_time__day AS DATETIME), INTERVAL 2 day) = subq_6.metric_time__day
      ) subq_10
    ) subq_11
  ) subq_12
) subq_13
