...
FROM (
  ...
  INNER JOIN (
    ...
    ON
      (
        TIMESTAMP_TRUNC(bookings_source_src_28000.ds, day) <= subq_19.ds
      ) AND (
        TIMESTAMP_TRUNC(bookings_source_src_28000.ds, day) > DATE_SUB(CAST(subq_19.ds AS DATETIME), INTERVAL 2 day)
      )
    GROUP BY
      metric_time__day
  ) subq_23
