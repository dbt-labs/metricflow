...
FROM (
  ...
  INNER JOIN (
    ...
    FROM (
      ...
      ON
        DATE_SUB(CAST(rss_28018_cte.ds__day AS DATETIME), INTERVAL 5 day) = subq_22.metric_time__day
    ) subq_28
  ) subq_29
  ON
    DATE_SUB(CAST(rss_28018_cte.ds__day AS DATETIME), INTERVAL 2 day) = subq_29.metric_time__day
) subq_34
