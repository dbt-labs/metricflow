...
FROM (
  ...
  FROM (
    ...
    FROM (
      ...
      INNER JOIN (
        ...
        FROM (
          ...
          SELECT
            ...
            , subq_3.__booking_value AS booking_value
          FROM (
      ...
      ON
        DATETIME_TRUNC(subq_8.metric_time__day, month) = subq_5.metric_time__day
    ) subq_9
    FULL OUTER JOIN (
      -- Compute Metrics via Expressions
      SELECT
      ...
      FROM (
        ...
        SELECT
          ...
          , subq_12.__booking_value AS booking_value
        FROM (
          -- Aggregate Inputs for Simple Metrics
          SELECT
            subq_11.metric_time__day
            , SUM(subq_11.__booking_value) AS __booking_value
          FROM (
            -- Pass Only Elements: ['__booking_value', 'metric_time__day']
            SELECT
