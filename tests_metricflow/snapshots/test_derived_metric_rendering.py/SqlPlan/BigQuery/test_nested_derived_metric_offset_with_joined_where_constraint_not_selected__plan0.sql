...
FROM (
  ...
  SELECT
    ...
    , nr_subq_13.bookings_offset_once
  FROM (
    -- Constrain Output with WHERE
    SELECT
      nr_subq_12.metric_time__day
      , nr_subq_12.booking__is_instant
      , nr_subq_12.bookings_offset_once
    FROM (
      -- Join to Time Spine Dataset
      SELECT
        nr_subq_11.metric_time__day AS metric_time__day
        , nr_subq_8.booking__is_instant AS booking__is_instant
        , nr_subq_8.bookings_offset_once AS bookings_offset_once
      FROM (
      ...
      ON
        DATE_SUB(CAST(nr_subq_11.metric_time__day AS DATETIME), INTERVAL 2 day) = nr_subq_8.metric_time__day
    ) nr_subq_12
