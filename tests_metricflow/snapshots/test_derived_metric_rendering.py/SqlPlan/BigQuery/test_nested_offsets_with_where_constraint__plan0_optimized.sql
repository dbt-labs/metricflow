  ...
  FROM (
    ...
    INNER JOIN (
      ...
      FROM (
        ...
        ON
          DATE_SUB(CAST(time_spine_src_28006.ds AS DATETIME), INTERVAL 5 day) = nr_subq_14.metric_time__day
        GROUP BY
          metric_time__day
      ) nr_subq_21
    ) nr_subq_22
    ON
      DATE_SUB(CAST(time_spine_src_28006.ds AS DATETIME), INTERVAL 2 day) = nr_subq_22.metric_time__day
  ) nr_subq_26
