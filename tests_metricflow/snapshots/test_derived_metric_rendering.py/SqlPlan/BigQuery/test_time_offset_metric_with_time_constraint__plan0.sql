...
FROM (
  ...
  SELECT
    ...
    , nr_subq_7.bookings AS bookings_5_days_ago
  FROM (
    -- Aggregate Measures
    SELECT
    ...
    FROM (
      ...
      FROM (
        ...
        FROM (
          ...
          ON
            DATE_SUB(CAST(nr_subq_3.metric_time__day AS DATETIME), INTERVAL 5 day) = nr_subq_0.metric_time__day
        ) nr_subq_4
        WHERE nr_subq_4.metric_time__day BETWEEN '2019-12-19' AND '2020-01-02'
      ) nr_subq_5
    ) nr_subq_6
    GROUP BY
      metric_time__day
