...
FROM (
  ...
  SELECT
    COALESCE(nr_subq_9.metric_time__day, nr_subq_13.metric_time__day, nr_subq_14.metric_time__day) AS metric_time__day
    , MAX(nr_subq_9.non_referred) AS non_referred
    , MAX(nr_subq_13.instant) AS instant
  ...
  FROM (
    ...
    FROM (
      ...
      SELECT
        ...
        , MAX(nr_subq_3.ref_bookings) AS ref_bookings
        , MAX(nr_subq_7.bookings) AS bookings
      FROM (
        -- Compute Metrics via Expressions
        SELECT
          nr_subq_2.metric_time__day
          , nr_subq_2.referred_bookings AS ref_bookings
        FROM (
          -- Aggregate Measures
          SELECT
            nr_subq_1.metric_time__day
            , SUM(nr_subq_1.referred_bookings) AS referred_bookings
          FROM (
  ...
  FULL OUTER JOIN (
    ...
    SELECT
      ...
      , nr_subq_12.instant_bookings AS instant
    FROM (
      -- Aggregate Measures
      SELECT
        nr_subq_11.metric_time__day
        , SUM(nr_subq_11.instant_bookings) AS instant_bookings
      FROM (
        -- Pass Only Elements: ['instant_bookings', 'metric_time__day']
        SELECT
          nr_subq_10.metric_time__day
          , nr_subq_10.instant_bookings
        FROM (
          -- Metric Time Dimension 'ds'
          SELECT
            nr_subq_28002.ds__day
  ...
  ON
    nr_subq_9.metric_time__day = nr_subq_13.metric_time__day
  FULL OUTER JOIN (
    -- Compute Metrics via Expressions
    SELECT
      nr_subq_6.metric_time__day
  ...
  ON
    COALESCE(nr_subq_9.metric_time__day, nr_subq_13.metric_time__day) = nr_subq_14.metric_time__day
  GROUP BY
    COALESCE(nr_subq_9.metric_time__day, nr_subq_13.metric_time__day, nr_subq_14.metric_time__day)
) nr_subq_15
