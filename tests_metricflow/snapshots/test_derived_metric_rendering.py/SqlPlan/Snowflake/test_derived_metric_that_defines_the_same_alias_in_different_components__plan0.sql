...
-- Order By [] Limit 1
SELECT
  nr_subq_10.booking__is_instant
  , nr_subq_10.derived_shared_alias_1a
  , nr_subq_10.derived_shared_alias_2
FROM (
  -- Combine Aggregated Outputs
  SELECT
    COALESCE(nr_subq_4.booking__is_instant, nr_subq_9.booking__is_instant) AS booking__is_instant
    , MAX(nr_subq_4.derived_shared_alias_1a) AS derived_shared_alias_1a
    , MAX(nr_subq_9.derived_shared_alias_2) AS derived_shared_alias_2
  FROM (
    ...
    FROM (
      ...
      SELECT
        ...
        , nr_subq_2.bookings AS shared_alias
      FROM (
        -- Aggregate Measures
        SELECT
  ...
  FULL OUTER JOIN (
    -- Compute Metrics via Expressions
    SELECT
      nr_subq_8.booking__is_instant
      , shared_alias + 10 AS derived_shared_alias_2
    FROM (
      -- Compute Metrics via Expressions
      SELECT
        nr_subq_7.booking__is_instant
        , nr_subq_7.instant_bookings AS shared_alias
      FROM (
        -- Aggregate Measures
        SELECT
          nr_subq_6.booking__is_instant
          , SUM(nr_subq_6.instant_bookings) AS instant_bookings
        FROM (
  ...
  ON
    nr_subq_4.booking__is_instant = nr_subq_9.booking__is_instant
  GROUP BY
    COALESCE(nr_subq_4.booking__is_instant, nr_subq_9.booking__is_instant)
) nr_subq_10
LIMIT 1
