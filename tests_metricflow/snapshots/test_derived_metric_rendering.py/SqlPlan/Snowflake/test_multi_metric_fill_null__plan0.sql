...
SELECT
  ...
  , subq_11.twice_bookings_fill_nulls_with_0_without_time_spine
  , subq_11.listings
FROM (
  ...
  FROM (
    ...
    FROM (
      ...
      SELECT
        ...
        , COALESCE(subq_3.__bookings_fill_nulls_with_0_without_time_spine, 0) AS bookings_fill_nulls_with_0_without_time_spine
      FROM (
        -- Aggregate Inputs for Simple Metrics
        SELECT
          subq_2.metric_time__day
          , SUM(subq_2.__bookings_fill_nulls_with_0_without_time_spine) AS __bookings_fill_nulls_with_0_without_time_spine
        FROM (
  ...
  FULL OUTER JOIN (
    ...
    SELECT
      ...
      , subq_9.__listings AS listings
    FROM (
      -- Aggregate Inputs for Simple Metrics
      SELECT
        subq_8.metric_time__day
        , SUM(subq_8.__listings) AS __listings
      FROM (
