...
SELECT
  ...
  , subq_16.every_2_days_bookers_2_days_ago
FROM (
  ...
  FROM (
    ...
    SELECT
      ...
      , subq_14.bookers AS every_2_days_bookers_2_days_ago
    FROM (
      -- Compute Metrics via Expressions
      SELECT
      ...
      FROM (
        ...
        FROM (
          ...
          ON
            DATE_ADD('day', -2, subq_11.metric_time__day) = subq_7.metric_time__day
        ) subq_12
        WHERE subq_12.metric_time__day BETWEEN timestamp '2019-12-19' AND timestamp '2020-01-02'
      ) subq_13
    ) subq_14
  ) subq_15
) subq_16
