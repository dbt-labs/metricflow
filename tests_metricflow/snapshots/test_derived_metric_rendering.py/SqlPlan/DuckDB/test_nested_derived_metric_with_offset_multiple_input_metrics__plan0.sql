...
SELECT
  ...
  , subq_20.booking_fees_since_start_of_month
FROM (
  ...
  FROM (
    ...
    SELECT
      COALESCE(subq_12.metric_time__day, subq_18.metric_time__day) AS metric_time__day
      , MAX(subq_12.booking_fees_start_of_month) AS booking_fees_start_of_month
      , MAX(subq_18.booking_fees) AS booking_fees
    FROM (
      -- Select: ['metric_time__day', 'booking_fees_start_of_month']
      SELECT
        subq_11.metric_time__day
        , subq_11.booking_fees_start_of_month
      FROM (
    ...
    FULL OUTER JOIN (
      ...
      FROM (
        ...
        SELECT
          ...
          , subq_16.__booking_value AS booking_value
        FROM (
          -- Aggregate Inputs for Simple Metrics
          SELECT
            subq_15.metric_time__day
            , SUM(subq_15.__booking_value) AS __booking_value
          FROM (
            -- Select: ['__booking_value', 'metric_time__day']
            SELECT
    ...
    ON
      subq_12.metric_time__day = subq_18.metric_time__day
    GROUP BY
      COALESCE(subq_12.metric_time__day, subq_18.metric_time__day)
  ) subq_19
) subq_20
