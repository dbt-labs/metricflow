  ...
  FROM (
    ...
    FROM (
      ...
      FROM (
          ...
          FROM (
            ...
            FROM (
              ...
              SELECT
                ...
                , EXTRACT(isodow FROM time_spine_src_16006.ds) AS ds__extract_dow
                , EXTRACT(doy FROM time_spine_src_16006.ds) AS ds__extract_doy
                , time_spine_src_16006.alien_day AS ds__alien_day
                , time_spine_src_16006.fiscal_quarter AS ds__fiscal_quarter
                , time_spine_src_16006.fiscal_year AS ds__fiscal_year
        ...
        ON
          subq_4.metric_time__month - INTERVAL 1 month = subq_1.metric_time__month
      ) subq_5
    ) subq_6
    GROUP BY
      subq_6.metric_time__month
