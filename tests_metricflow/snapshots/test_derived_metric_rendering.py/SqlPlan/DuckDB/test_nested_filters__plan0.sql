...
-- Write to DataTable
SELECT
  subq_29.instant_lux_booking_value_rate
FROM (
  ...
  FROM (
    ...
    FROM (
      ...
      CROSS JOIN (
        -- Compute Metrics via Expressions
        SELECT
          subq_25.__booking_value AS booking_value
        FROM (
          -- Aggregate Inputs for Simple Metrics
          SELECT
            SUM(subq_24.__booking_value) AS __booking_value
          FROM (
            -- Select: ['__booking_value']
            SELECT
              subq_23.__booking_value
            FROM (
              -- Constrain Output with WHERE
              SELECT
                subq_22.booking_value AS __booking_value
