...
FROM (
  ...
  FROM (
    ...
    SELECT
      COALESCE(subq_5.metric_time__day, subq_10.metric_time__day, subq_18.metric_time__day) AS metric_time__day
      , MAX(subq_5.bookings) AS bookings
      , MAX(subq_10.instant) AS instant
      , MAX(subq_18.non_referred) AS non_referred
    FROM (
      -- Compute Metrics via Expressions
      SELECT
    ...
    FULL OUTER JOIN (
      ...
      SELECT
        ...
        , subq_9.__instant_bookings AS instant
      FROM (
        -- Aggregate Inputs for Simple Metrics
        SELECT
          subq_8.metric_time__day
          , SUM(subq_8.__instant_bookings) AS __instant_bookings
        FROM (
    ...
    FULL OUTER JOIN (
      ...
      FROM (
        -- Combine Aggregated Outputs
        SELECT
          COALESCE(subq_15.metric_time__day, subq_16.metric_time__day) AS metric_time__day
          , MAX(subq_15.ref_bookings) AS ref_bookings
          , MAX(subq_16.bookings) AS bookings
        FROM (
          -- Compute Metrics via Expressions
          SELECT
            subq_14.metric_time__day
            , subq_14.__referred_bookings AS ref_bookings
          FROM (
            -- Aggregate Inputs for Simple Metrics
            SELECT
              subq_13.metric_time__day
              , SUM(subq_13.__referred_bookings) AS __referred_bookings
            FROM (
        ...
        GROUP BY
          COALESCE(subq_15.metric_time__day, subq_16.metric_time__day)
      ) subq_17
    ) subq_18
    ON
      COALESCE(subq_5.metric_time__day, subq_10.metric_time__day) = subq_18.metric_time__day
    GROUP BY
      COALESCE(subq_5.metric_time__day, subq_10.metric_time__day, subq_18.metric_time__day)
  ) subq_19
) subq_20
