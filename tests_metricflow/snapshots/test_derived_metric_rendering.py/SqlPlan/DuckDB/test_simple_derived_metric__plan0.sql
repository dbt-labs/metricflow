...
-- Write to DataTable
SELECT
  subq_18.test_simple_derived_metric
FROM (
  ...
  FROM (
    ...
    SELECT
      MAX(subq_4.alias_1) AS alias_1
      , MAX(subq_8.alias_2) AS alias_2
      , MAX(subq_12.alias_3) AS alias_3
      , MAX(subq_16.alias_4) AS alias_4
    FROM (
      -- Compute Metrics via Expressions
      SELECT
        subq_3.__bookings AS alias_1
      FROM (
        -- Aggregate Inputs for Simple Metrics
        SELECT
          SUM(subq_2.__bookings) AS __bookings
        FROM (
    ...
    CROSS JOIN (
      -- Compute Metrics via Expressions
      SELECT
        subq_7.__referred_bookings AS alias_2
      FROM (
        -- Aggregate Inputs for Simple Metrics
        SELECT
          SUM(subq_6.__referred_bookings) AS __referred_bookings
        FROM (
    ...
    CROSS JOIN (
      -- Compute Metrics via Expressions
      SELECT
        subq_11.__instant_bookings AS alias_3
      FROM (
        -- Aggregate Inputs for Simple Metrics
        SELECT
          SUM(subq_10.__instant_bookings) AS __instant_bookings
        FROM (
    ...
    CROSS JOIN (
      -- Compute Metrics via Expressions
      SELECT
        subq_15.__booking_value AS alias_4
      FROM (
        -- Aggregate Inputs for Simple Metrics
        SELECT
