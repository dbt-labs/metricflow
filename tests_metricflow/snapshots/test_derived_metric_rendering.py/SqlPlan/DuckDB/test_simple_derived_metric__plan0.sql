...
-- Write to DataTable
SELECT
  subq_22.test_simple_derived_metric
FROM (
  ...
  FROM (
    ...
    SELECT
      MAX(subq_5.alias_1) AS alias_1
      , MAX(subq_10.alias_2) AS alias_2
      , MAX(subq_15.alias_3) AS alias_3
      , MAX(subq_20.alias_4) AS alias_4
    FROM (
      -- Compute Metrics via Expressions
      SELECT
        subq_4.__bookings AS alias_1
      FROM (
    ...
    CROSS JOIN (
      -- Compute Metrics via Expressions
      SELECT
        subq_9.__referred_bookings AS alias_2
      FROM (
        -- Aggregate Inputs for Simple Metrics
        SELECT
    ...
    CROSS JOIN (
      -- Compute Metrics via Expressions
      SELECT
        subq_14.__instant_bookings AS alias_3
      FROM (
        -- Aggregate Inputs for Simple Metrics
        SELECT
    ...
    CROSS JOIN (
      -- Compute Metrics via Expressions
      SELECT
        subq_19.__booking_value AS alias_4
      FROM (
        -- Aggregate Inputs for Simple Metrics
        SELECT
