...
FROM (
  ...
  SELECT
    COALESCE(nr_subq_3.booking__ds__day, nr_subq_11.booking__ds__day) AS booking__ds__day
    , MAX(nr_subq_3.bookings) AS bookings
    , MAX(nr_subq_11.bookings_at_start_of_month) AS bookings_at_start_of_month
  FROM (
    -- Compute Metrics via Expressions
    SELECT
  ...
  FULL OUTER JOIN (
    ...
    SELECT
      ...
      , nr_subq_10.bookings AS bookings_at_start_of_month
    FROM (
      -- Aggregate Measures
      SELECT
        ...
        FROM (
          ...
          FROM (
            ...
            FROM (
              ...
              SELECT
                nr_subq_5.ds__day AS booking__ds__day
                , nr_subq_5.ds__week
                , nr_subq_5.ds__month
          ...
          ON
            DATE_TRUNC('month', nr_subq_7.booking__ds__day) = nr_subq_4.booking__ds__day
        ) nr_subq_8
      ) nr_subq_9
      GROUP BY
        nr_subq_9.booking__ds__day
  ...
  ON
    nr_subq_3.booking__ds__day = nr_subq_11.booking__ds__day
  GROUP BY
    COALESCE(nr_subq_3.booking__ds__day, nr_subq_11.booking__ds__day)
) nr_subq_12
