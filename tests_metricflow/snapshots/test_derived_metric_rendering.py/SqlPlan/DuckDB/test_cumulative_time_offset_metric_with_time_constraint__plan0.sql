...
SELECT
  ...
  , subq_18.every_2_days_bookers_2_days_ago
FROM (
  ...
  FROM (
    ...
    SELECT
      ...
      , subq_16.bookers AS every_2_days_bookers_2_days_ago
    FROM (
      -- Compute Metrics via Expressions
      SELECT
        subq_15.metric_time__day
        , subq_15.__bookers AS bookers
      FROM (
        ...
        FROM (
          ...
          ON
            subq_13.metric_time__day - INTERVAL 2 day = subq_8.metric_time__day
        ) subq_14
        WHERE subq_14.metric_time__day BETWEEN '2019-12-19' AND '2020-01-02'
      ) subq_15
    ) subq_16
  ) subq_17
) subq_18
