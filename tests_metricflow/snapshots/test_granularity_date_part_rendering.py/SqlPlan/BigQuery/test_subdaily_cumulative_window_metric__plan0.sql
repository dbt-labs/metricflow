...
SELECT
  ...
  , nr_subq_5.archived_users AS subdaily_cumulative_window_metric
FROM (
  -- Aggregate Measures
  SELECT
    nr_subq_4.metric_time__hour
    , SUM(nr_subq_4.archived_users) AS archived_users
  FROM (
    ...
    FROM (
      ...
      SELECT
        nr_subq_1.metric_time__hour AS metric_time__hour
      ...
      FROM (
        ...
        SELECT
          nr_subq_2.ts AS metric_time__hour
        FROM ***************************.mf_time_spine_hour nr_subq_2
      ) nr_subq_1
      INNER JOIN (
      ...
      ON
        (
          nr_subq_0.metric_time__hour <= nr_subq_1.metric_time__hour
        ) AND (
          nr_subq_0.metric_time__hour > DATE_SUB(CAST(nr_subq_1.metric_time__hour AS DATETIME), INTERVAL 3 hour)
        )
    ) nr_subq_3
  ) nr_subq_4
  GROUP BY
