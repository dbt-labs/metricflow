...
FROM (
  ...
  FROM (
    ...
    SELECT
      ...
      , subq_7.__archived_users AS archived_users
    FROM (
      -- Join to Time Spine Dataset
      SELECT
        subq_6.metric_time__hour AS metric_time__hour
        , subq_3.__archived_users AS __archived_users
      FROM (
      ...
      INNER JOIN (
        ...
        SELECT
          ...
          , SUM(subq_2.__archived_users) AS __archived_users
        FROM (
      ...
      ON
        DATETIME_TRUNC(subq_6.metric_time__hour, hour) = subq_3.metric_time__hour
    ) subq_7
  ) subq_8
) subq_9
