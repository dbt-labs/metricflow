...
SELECT
  ...
  , subq_8.subdaily_cumulative_grain_to_date_metric
FROM (
  -- Compute Metrics via Expressions
  SELECT
    subq_7.metric_time__hour
    , subq_7.simple_subdaily_metric_default_day AS subdaily_cumulative_grain_to_date_metric
  FROM (
    -- Compute Metrics via Expressions
    SELECT
      subq_6.metric_time__hour
      , subq_6.simple_subdaily_metric_default_day
    FROM (
      -- Aggregate Inputs for Simple Metrics
      SELECT
        subq_5.metric_time__hour
        , SUM(subq_5.simple_subdaily_metric_default_day) AS simple_subdaily_metric_default_day
      FROM (
        -- Pass Only Elements: ['simple_subdaily_metric_default_day', 'metric_time__hour']
        SELECT
          subq_4.metric_time__hour
          , subq_4.simple_subdaily_metric_default_day
        FROM (
          ...
          ON
            (
              ...
              subq_1.metric_time__hour >= DATETIME_TRUNC(subq_2.metric_time__hour, hour)
            )
        ) subq_4
      ) subq_5
      GROUP BY
