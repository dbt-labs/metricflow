...
FROM (
  ...
  FULL OUTER JOIN (
    ...
    -- Compute Metrics via Expressions
    SELECT
      IF(EXTRACT(DOW FROM time_spine_src_28006.ds) = 1, 7, EXTRACT(DOW FROM time_spine_src_28006.ds) - 1) AS metric_time__extract_dow
    ...
    GROUP BY
      IF(EXTRACT(DOW FROM time_spine_src_28006.ds) = 1, 7, EXTRACT(DOW FROM time_spine_src_28006.ds) - 1)
  ) subq_33
  ON
