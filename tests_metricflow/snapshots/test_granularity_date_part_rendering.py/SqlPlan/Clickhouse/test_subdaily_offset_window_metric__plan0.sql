  ...
  FROM (
    ...
    FROM (
      ...
      FROM (
          ...
          FROM (
            ...
            FROM (
              ...
              SELECT
                ...
                , EXTRACT(toYear FROM time_spine_src_28005.ts) AS ts__extract_year
                , EXTRACT(toQuarter FROM time_spine_src_28005.ts) AS ts__extract_quarter
                , EXTRACT(toMonth FROM time_spine_src_28005.ts) AS ts__extract_month
                , EXTRACT(toDayOfMonth FROM time_spine_src_28005.ts) AS ts__extract_day
                , EXTRACT(toDayOfWeek FROM time_spine_src_28005.ts) AS ts__extract_dow
                , EXTRACT(toDayOfYear FROM time_spine_src_28005.ts) AS ts__extract_doy
        ...
        ON
          addHours(subq_4.metric_time__hour, CAST(-1 AS Integer)) = subq_1.metric_time__hour
        SETTINGS allow_experimental_join_condition = 1, allow_experimental_analyzer = 1, join_use_nulls = 0
      ) subq_5
      SETTINGS allow_experimental_join_condition = 1, allow_experimental_analyzer = 1, join_use_nulls = 0
    ) subq_6
