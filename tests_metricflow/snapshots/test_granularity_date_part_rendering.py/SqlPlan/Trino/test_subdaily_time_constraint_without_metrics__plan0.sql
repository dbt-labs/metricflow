  ...
  FROM (
    ...
    FROM (
      ...
      SELECT
        ...
        , EXTRACT(DAY_OF_WEEK FROM time_spine_src_28003.ts) AS ts__extract_dow
        , EXTRACT(doy FROM time_spine_src_28003.ts) AS ts__extract_doy
      FROM ***************************.mf_time_spine_second time_spine_src_28003
    ) nr_subq_28016
  ) nr_subq_0
  WHERE nr_subq_0.metric_time__second BETWEEN timestamp '2020-01-01 00:00:02' AND timestamp '2020-01-01 00:00:08'
) nr_subq_1
GROUP BY
  nr_subq_1.metric_time__second
