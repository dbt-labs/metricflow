    ...
    FROM (
      ...
      FROM (
        ...
        SELECT
          ...
          , CASE WHEN EXTRACT(DAYOFWEEK FROM time_spine_src_28002.ts) = 1 THEN 7 ELSE EXTRACT(DAYOFWEEK FROM time_spine_src_28002.ts) - 1 END AS ts__extract_dow
          , DAYOFYEAR(time_spine_src_28002.ts) AS ts__extract_doy
