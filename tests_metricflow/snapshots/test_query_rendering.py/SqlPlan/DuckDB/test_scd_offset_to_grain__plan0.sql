...
SELECT
  ...
  , subq_14.listing__capacity
  , subq_14.bookings_all_time
FROM (
  -- Re-aggregate Metric via Group By
  SELECT
    subq_13.metric_time__month
    , subq_13.listing__capacity
    , subq_13.bookings_all_time
  FROM (
    -- Window Function for Metric Re-aggregation
    SELECT
      subq_12.metric_time__month
      , subq_12.listing__capacity
      , LAST_VALUE(subq_12.bookings_all_time) OVER (
        PARTITION BY
          subq_12.listing__capacity
          , subq_12.metric_time__month
    ...
    FROM (
      ...
      SELECT
        ...
        , subq_11.listing__capacity
        , subq_11.bookings AS bookings_all_time
      FROM (
        -- Compute Metrics via Expressions
        SELECT
          subq_10.metric_time__day
          , subq_10.metric_time__month
          , subq_10.listing__capacity
          ...
          FROM (
            ...
            FROM (
              ...
              FROM (
                ...
                SELECT
                  subq_6.capacity AS listing__capacity
                ...
                ON
                  (
                    ...
                    (
                      subq_4.metric_time__day >= subq_6.window_start__day
                    ) AND (
                      (
                        subq_4.metric_time__day < subq_6.window_end__day
                      ) OR (
