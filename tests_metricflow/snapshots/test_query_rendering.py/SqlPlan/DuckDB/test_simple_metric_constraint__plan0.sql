...
SELECT
  ...
  , subq_27.lux_booking_value_rate_expr
FROM (
  ...
  FROM (
    ...
    FROM (
      -- Combine Aggregated Outputs
      SELECT
        COALESCE(subq_10.metric_time__day, subq_19.metric_time__day) AS metric_time__day
      ...
      GROUP BY
        COALESCE(subq_10.metric_time__day, subq_19.metric_time__day)
    ) subq_20
    FULL OUTER JOIN (
      -- Compute Metrics via Expressions
      SELECT
