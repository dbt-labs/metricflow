...
FROM (
  ...
  SELECT
    COALESCE(subq_9.metric_time__day, subq_17.metric_time__day, subq_25.metric_time__day) AS metric_time__day
      ...
      FROM (
        ...
        FROM (
          ...
          FROM (
            -- Join Standard Outputs
            SELECT
              subq_20.is_lux_latest AS listing__is_lux_latest
  ...
  ON
    COALESCE(subq_9.metric_time__day, subq_17.metric_time__day) = subq_25.metric_time__day
  GROUP BY
    COALESCE(subq_9.metric_time__day, subq_17.metric_time__day, subq_25.metric_time__day)
) subq_26
