  ...
  FROM (
    ...
    FROM (
      ...
      SELECT
        ...
        , nr_subq_21.listing__views_times_booking_value
        , nr_subq_21.listings
      ...
      FROM (
        ...
        SELECT
          nr_subq_20.listing__views_times_booking_value AS listing__views_times_booking_value
        ...
        LEFT OUTER JOIN (
          ...
          SELECT
            nr_subq_19.listing
            , nr_subq_19.listing__views_times_booking_value
          FROM (
            -- Compute Metrics via Expressions
            SELECT
              nr_subq_18.listing
            ...
            FROM (
              ...
              SELECT
                ...
                , MAX(nr_subq_13.booking_value) AS booking_value
                , MAX(nr_subq_17.views) AS views
              FROM (
                -- Compute Metrics via Expressions
                SELECT
                  nr_subq_12.listing
                  , nr_subq_12.booking_value
                FROM (
                  -- Aggregate Measures
                  SELECT
                    nr_subq_11.listing
                    , SUM(nr_subq_11.booking_value) AS booking_value
                  FROM (
                    -- Pass Only Elements: ['booking_value', 'listing']
                    SELECT
                      nr_subq_10.listing
                      , nr_subq_10.booking_value
                    FROM (
                      -- Metric Time Dimension 'ds'
                      SELECT
