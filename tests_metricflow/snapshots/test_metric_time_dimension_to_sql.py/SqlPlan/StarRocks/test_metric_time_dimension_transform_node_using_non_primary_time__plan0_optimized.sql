...
SELECT
  ...
  , CASE WHEN EXTRACT(DAYOFWEEK FROM ds) = 1 THEN 7 ELSE EXTRACT(DAYOFWEEK FROM ds) - 1 END AS ds__extract_dow
  , DAYOFYEAR(ds) AS ds__extract_doy
  ...
  , CASE WHEN EXTRACT(DAYOFWEEK FROM ds_partitioned) = 1 THEN 7 ELSE EXTRACT(DAYOFWEEK FROM ds_partitioned) - 1 END AS ds_partitioned__extract_dow
  , DAYOFYEAR(ds_partitioned) AS ds_partitioned__extract_doy
  ...
  , CASE WHEN EXTRACT(DAYOFWEEK FROM paid_at) = 1 THEN 7 ELSE EXTRACT(DAYOFWEEK FROM paid_at) - 1 END AS paid_at__extract_dow
  , DAYOFYEAR(paid_at) AS paid_at__extract_doy
  ...
  , CASE WHEN EXTRACT(DAYOFWEEK FROM ds) = 1 THEN 7 ELSE EXTRACT(DAYOFWEEK FROM ds) - 1 END AS booking__ds__extract_dow
  , DAYOFYEAR(ds) AS booking__ds__extract_doy
  ...
  , CASE WHEN EXTRACT(DAYOFWEEK FROM ds_partitioned) = 1 THEN 7 ELSE EXTRACT(DAYOFWEEK FROM ds_partitioned) - 1 END AS booking__ds_partitioned__extract_dow
  , DAYOFYEAR(ds_partitioned) AS booking__ds_partitioned__extract_doy
  ...
  , CASE WHEN EXTRACT(DAYOFWEEK FROM paid_at) = 1 THEN 7 ELSE EXTRACT(DAYOFWEEK FROM paid_at) - 1 END AS booking__paid_at__extract_dow
  , DAYOFYEAR(paid_at) AS booking__paid_at__extract_doy
  ...
  , CASE WHEN EXTRACT(DAYOFWEEK FROM paid_at) = 1 THEN 7 ELSE EXTRACT(DAYOFWEEK FROM paid_at) - 1 END AS metric_time__extract_dow
  , DAYOFYEAR(paid_at) AS metric_time__extract_doy
