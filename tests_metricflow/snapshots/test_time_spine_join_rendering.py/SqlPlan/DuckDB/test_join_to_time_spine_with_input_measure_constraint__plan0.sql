...
FROM (
  ...
  SELECT
    ...
    , subq_4.bookings AS instant_bookings_with_measure_filter
  FROM (
    -- Aggregate Measures
    SELECT
    ...
    FROM (
      ...
      FROM (
        ...
        WHERE (((booking__is_instant) AND (listing IS NOT NULL)) AND (booking__is_instant)) AND (metric_time__day > '2020-01-01')
      ) subq_2
    ) subq_3
    GROUP BY
      subq_3.metric_time__day
