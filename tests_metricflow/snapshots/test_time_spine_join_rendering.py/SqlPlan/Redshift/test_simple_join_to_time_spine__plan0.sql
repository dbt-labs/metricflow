...
FROM (
  ...
  SELECT
    ...
    , subq_7.bookings_join_to_time_spine
  FROM (
    -- Join to Time Spine Dataset
    SELECT
      subq_6.metric_time__day AS metric_time__day
      , subq_3.bookings_join_to_time_spine AS bookings_join_to_time_spine
    FROM (
    ...
    LEFT OUTER JOIN (
      ...
      SELECT
        ...
        , SUM(subq_2.bookings_join_to_time_spine) AS bookings_join_to_time_spine
      FROM (
        -- Pass Only Elements: ['bookings_join_to_time_spine', 'metric_time__day']
        SELECT
