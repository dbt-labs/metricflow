...
SELECT
  ...
  , subq_26.bookings_growth_2_weeks
FROM (
  ...
  FROM (
    ...
    SELECT
      ...
      , MAX(subq_24.bookings_2_weeks_ago) AS bookings_2_weeks_ago
    FROM (
      -- Compute Metrics via Expressions
      SELECT
    ...
    FULL OUTER JOIN (
      ...
      SELECT
        ...
        , subq_23.__bookings AS bookings_2_weeks_ago
      FROM (
        -- Join to Time Spine Dataset
        SELECT
          subq_22.metric_time__day AS metric_time__day
          , subq_18.listing__country_latest AS listing__country_latest
          , subq_18.__bookings AS __bookings
        FROM (
        ...
        ON
          DATE_SUB(subq_22.metric_time__day, INTERVAL 14 DAY) = subq_18.metric_time__day
      ) subq_23
    ) subq_24
    ON
      (
