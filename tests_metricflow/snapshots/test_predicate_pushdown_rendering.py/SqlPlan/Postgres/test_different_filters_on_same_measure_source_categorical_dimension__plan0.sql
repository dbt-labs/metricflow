...
SELECT
  ...
  , CAST(nr_subq_9.average_booking_value AS DOUBLE PRECISION) / CAST(NULLIF(nr_subq_9.max_booking_value, 0) AS DOUBLE PRECISION) AS instant_booking_fraction_of_max_value
FROM (
  -- Combine Aggregated Outputs
  SELECT
    COALESCE(nr_subq_4.metric_time__day, nr_subq_8.metric_time__day) AS metric_time__day
    , MAX(nr_subq_4.average_booking_value) AS average_booking_value
    , MAX(nr_subq_8.max_booking_value) AS max_booking_value
  FROM (
    -- Compute Metrics via Expressions
    SELECT
    ...
    FROM (
      ...
      SELECT
        ...
        , AVG(nr_subq_2.average_booking_value) AS average_booking_value
      FROM (
  ...
  FULL OUTER JOIN (
    ...
    FROM (
      ...
      SELECT
        ...
        , MAX(nr_subq_6.max_booking_value) AS max_booking_value
      FROM (
  ...
  GROUP BY
    COALESCE(nr_subq_4.metric_time__day, nr_subq_8.metric_time__day)
) nr_subq_9
