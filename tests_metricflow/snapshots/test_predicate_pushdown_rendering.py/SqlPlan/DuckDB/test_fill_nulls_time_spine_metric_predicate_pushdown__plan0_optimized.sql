...
FROM (
  ...
  SELECT
    ...
    , MAX(subq_33.bookings_fill_nulls_with_0) AS bookings_fill_nulls_with_0
  ...
  FULL OUTER JOIN (
    ...
    ON
      time_spine_src_28006.ds - INTERVAL 14 day = subq_40.metric_time__day
  ) subq_45
  ON
    (
  ...
  GROUP BY
    COALESCE(subq_33.metric_time__day, subq_45.metric_time__day)
    , COALESCE(subq_33.listing__country_latest, subq_45.listing__country_latest)
) subq_46
