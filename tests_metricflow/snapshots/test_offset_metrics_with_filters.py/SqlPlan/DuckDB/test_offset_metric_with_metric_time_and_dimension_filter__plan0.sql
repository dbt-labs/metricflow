test_name: test_offset_metric_with_metric_time_and_dimension_filter
test_filename: test_offset_metrics_with_filters.py
sql_engine: DuckDB
expectation_description:
  The metric_time portion of the filter should be applied on the time spine /
  output side of the offset join, ideally by pushing it to the time spine before
  the join, while the dimension portion should stay on the pre-offset metric
  input.
---
-- Write to DataTable
SELECT
...
FROM (
  ...
  FROM (
    ...
    FROM (
      ...
      INNER JOIN (
        ...
        FROM (
          ...
          FROM (
            ...
            WHERE metric_time__day = '2020-01-01' AND listing__country_latest == 'us'
          ) subq_7
        ) subq_8
        GROUP BY
          subq_8.metric_time__day
      ...
      ON
        subq_13.metric_time__day - INTERVAL 5 day = subq_9.metric_time__day
    ) subq_14
  ) subq_15
) subq_16
