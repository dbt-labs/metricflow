...
SELECT
  ...
  , subq_22.bookings_growth_2_weeks_fill_nulls_with_0_for_non_offset
FROM (
  ...
  FROM (
    ...
    FROM (
      ...
      SELECT
        ...
        , COALESCE(subq_9.__bookings_fill_nulls_with_0, 0) AS bookings_fill_nulls_with_0
      FROM (
    ...
    FULL OUTER JOIN (
      ...
      SELECT
        ...
        , subq_19.__bookings AS bookings_2_weeks_ago
      FROM (
        -- Join to Time Spine Dataset
        SELECT
        ...
        ON
          DATE_SUB(subq_18.metric_time__day, INTERVAL 14 DAY) = subq_14.metric_time__day
      ) subq_19
    ) subq_20
    ON
