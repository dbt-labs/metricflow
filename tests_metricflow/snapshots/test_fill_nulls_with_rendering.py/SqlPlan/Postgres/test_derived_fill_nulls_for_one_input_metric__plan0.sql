...
FROM (
  ...
  SELECT
    COALESCE(nr_subq_7.metric_time__day, nr_subq_15.metric_time__day) AS metric_time__day
    , COALESCE(MAX(nr_subq_7.bookings_fill_nulls_with_0), 0) AS bookings_fill_nulls_with_0
    , MAX(nr_subq_15.bookings_2_weeks_ago) AS bookings_2_weeks_ago
  FROM (
    -- Compute Metrics via Expressions
    SELECT
      nr_subq_6.metric_time__day
      , COALESCE(nr_subq_6.bookings, 0) AS bookings_fill_nulls_with_0
    FROM (
      -- Join to Time Spine Dataset
      SELECT
        nr_subq_5.metric_time__day AS metric_time__day
        , nr_subq_2.bookings AS bookings
      FROM (
  ...
  FULL OUTER JOIN (
    ...
    SELECT
      ...
      , nr_subq_14.bookings AS bookings_2_weeks_ago
    FROM (
      -- Aggregate Measures
      SELECT
  ...
  GROUP BY
    COALESCE(nr_subq_7.metric_time__day, nr_subq_15.metric_time__day)
) nr_subq_16
