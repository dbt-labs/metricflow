...
FROM (
  ...
  FROM (
    ...
    SELECT
      ...
      , AVG(subq_8.trailing_2_months_revenue) OVER (PARTITION BY subq_8.metric_time__year) AS trailing_2_months_revenue
    FROM (
      -- Compute Metrics via Expressions
      SELECT
        subq_7.metric_time__day
        , subq_7.metric_time__year
        , subq_7.revenue AS trailing_2_months_revenue
      FROM (
        -- Compute Metrics via Expressions
        SELECT
