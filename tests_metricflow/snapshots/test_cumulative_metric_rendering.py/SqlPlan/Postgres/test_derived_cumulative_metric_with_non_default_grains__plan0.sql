...
FROM (
  ...
  FROM (
    ...
    SELECT
      ...
      , AVG(nr_subq_6.t2mr) OVER (PARTITION BY nr_subq_6.metric_time__week) AS t2mr
    FROM (
      -- Compute Metrics via Expressions
      SELECT
        nr_subq_5.metric_time__day
        , nr_subq_5.metric_time__week
        , nr_subq_5.txn_revenue AS t2mr
      FROM (
        -- Aggregate Measures
        SELECT
          ...
          FROM (
            ...
            ON
              (
                ...
                nr_subq_0.metric_time__day > nr_subq_1.metric_time__day - MAKE_INTERVAL(months => 2)
              )
          ) nr_subq_3
        ) nr_subq_4
        GROUP BY
