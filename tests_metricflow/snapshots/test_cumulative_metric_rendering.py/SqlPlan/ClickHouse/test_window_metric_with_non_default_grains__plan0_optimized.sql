...
FROM (
  ...
  FROM (
    SELECT
      ...
      , toStartOfMonth(time_spine_src_28006.ds) AS booking__ds__month
      , subq_24.__bookers_fill_nulls_with_0_join_to_timespine AS __bookers_fill_nulls_with_0_join_to_timespine
    FROM ***************************.mf_time_spine time_spine_src_28006
    LEFT OUTER JOIN (
      SELECT
        toStartOfMonth(subq_20.ds) AS booking__ds__month
        , subq_20.ds AS metric_time__day
        , toStartOfWeek(subq_20.ds, 1) AS metric_time__week
      ...
      ON
        (
          toStartOfDay(bookings_source_src_28000.ds) <= subq_20.ds
        ) AND (
          toStartOfDay(bookings_source_src_28000.ds) > addDays(subq_20.ds, -2)
        )
      GROUP BY
        toStartOfMonth(subq_20.ds)
        , subq_20.ds
        , toStartOfWeek(subq_20.ds, 1)
    ) subq_24
    ON
