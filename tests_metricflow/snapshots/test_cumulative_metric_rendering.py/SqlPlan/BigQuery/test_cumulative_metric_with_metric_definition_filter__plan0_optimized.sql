...
FROM (
  ...
  FROM (
    ...
    ON
      (
        TIMESTAMP_TRUNC(revenue_src_28000.created_at, day) <= subq_17.ds
      ) AND (
        TIMESTAMP_TRUNC(revenue_src_28000.created_at, day) > DATE_SUB(CAST(subq_17.ds AS DATETIME), INTERVAL 2 month)
      )
  ) subq_18
  LEFT OUTER JOIN
