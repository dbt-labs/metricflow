...
FROM (
  ...
  ON
    (
      DATETIME_TRUNC(bookings_source_src_28000.ds, day) <= nr_subq_9.ds
    ) AND (
      DATETIME_TRUNC(bookings_source_src_28000.ds, day) > DATE_SUB(CAST(nr_subq_9.ds AS DATETIME), INTERVAL 2 day)
    )
) nr_subq_10
