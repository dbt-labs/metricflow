...
FROM (
  ...
  FROM (
    ...
    LEFT OUTER JOIN (
      ...
      SELECT
        TIMESTAMP_TRUNC(subq_20.ds, month) AS booking__ds__month
        , subq_20.ds AS metric_time__day
        , TIMESTAMP_TRUNC(subq_20.ds, isoweek) AS metric_time__week
      ...
      ON
        (
          TIMESTAMP_TRUNC(bookings_source_src_28000.ds, day) <= subq_20.ds
        ) AND (
          TIMESTAMP_TRUNC(bookings_source_src_28000.ds, day) > DATE_SUB(CAST(subq_20.ds AS DATETIME), INTERVAL 2 day)
        )
      GROUP BY
        booking__ds__month
        , metric_time__day
