...
SELECT
  ...
  , subq_15.trailing_2_months_revenue
FROM (
  -- Compute Metrics via Expressions
  SELECT
    subq_14.metric_time__day
    , subq_14.revenue AS trailing_2_months_revenue
  FROM (
    -- Compute Metrics via Expressions
    SELECT
          ...
          FROM (
            ...
            FROM (
              ...
              INNER JOIN (
                ...
                WHERE subq_5.metric_time__day BETWEEN '2019-11-01' AND '2020-01-01'
              ) subq_6
              ON
                (
                  subq_6.metric_time__day <= subq_7.metric_time__day
                ) AND (
                  subq_6.metric_time__day > DATE_SUB(CAST(subq_7.metric_time__day AS DATETIME), INTERVAL 2 month)
                )
            ) subq_9
          ) subq_10
