...
SELECT
  ...
  , subq_14.visit_buy_conversion_rate
FROM (
  -- Compute Metrics via Expressions
  SELECT
    subq_13.visit__referrer_id
    , CAST(subq_13.__buys AS DOUBLE PRECISION) / CAST(NULLIF(subq_13.__visits, 0) AS DOUBLE PRECISION) AS visit_buy_conversion_rate
  FROM (
