...
-- Write to DataTable
SELECT
  subq_14.visit_buy_conversions
FROM (
  -- Compute Metrics via Expressions
  SELECT
    subq_13.__buys_fill_nulls_with_0 AS visit_buy_conversions
  FROM (
    -- Combine Aggregated Outputs
    SELECT
      MAX(subq_3.__visits) AS __visits
      , COALESCE(MAX(subq_12.__buys_fill_nulls_with_0), 0) AS __buys_fill_nulls_with_0
    FROM (
    ...
    CROSS JOIN (
      -- Aggregate Inputs for Simple Metrics
      SELECT
        SUM(subq_11.__buys_fill_nulls_with_0) AS __buys_fill_nulls_with_0
      FROM (
        ...
        FROM (
          ...
          FROM (
            ...
            SELECT DISTINCT
              ...
              , subq_8.__buys_fill_nulls_with_0 AS __buys_fill_nulls_with_0
            FROM (
            ...
            ON
              (
                ...
                (
                  ...
                  subq_5.metric_time__day > subq_8.metric_time__day - MAKE_INTERVAL(days => 7)
                )
              )
          ) subq_9
        ) subq_10
