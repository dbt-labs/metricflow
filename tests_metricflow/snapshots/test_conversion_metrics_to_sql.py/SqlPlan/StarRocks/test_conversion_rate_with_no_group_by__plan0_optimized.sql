...
SELECT
  CAST(MAX(subq_33.__buys) AS DOUBLE) / CAST(NULLIF(MAX(subq_22.__visits), 0) AS DOUBLE) AS visit_buy_conversion_rate_7days
FROM (
    ...
    ON
      (
        ...
        (
          ...
          sma_28019_cte.metric_time__day > DATE_SUB(subq_28.metric_time__day, INTERVAL 7 DAY)
        )
      )
  ) subq_29
) subq_33
