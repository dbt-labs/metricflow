...
SELECT
  CAST(MAX(nr_subq_10.buys) AS DOUBLE) / CAST(NULLIF(MAX(nr_subq_2.visits), 0) AS DOUBLE) AS visit_buy_conversion_rate_7days
FROM (
    ...
    ON
      (
        ...
        (
          ...
          nr_subq_4.metric_time__day > nr_subq_6.metric_time__day - INTERVAL 7 day
        )
      )
  ) nr_subq_7
) nr_subq_10
