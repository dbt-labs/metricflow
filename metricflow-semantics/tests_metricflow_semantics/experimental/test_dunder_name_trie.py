...
def test_add_items(request: FixtureRequest, mf_test_configuration: MetricFlowTestConfiguration) -> None:  # noqa: D103
    ...
    for indexed_dunder_name in indexed_dunder_names:
        dunder_name_trie.add_name_item(indexed_dunder_name, descriptor)
    ...
    assert_object_snapshot_equal(
        ...
        expectation_description=mf_dedent(
            """
            For the `ambiguous` name, there should be a trie with empty fields to indicate an ambiguous item.
            """
        ),
    )
...
def test_union() -> None:  # noqa: D103
    ...
    assert MutableDunderNameTrie.union([left_trie, right_trie]).dunder_names() == ("listing", "listing__country")
...
def test_intersection() -> None:  # noqa: D103
    ...
    assert MutableDunderNameTrie.intersection([left_trie, right_trie]).dunder_names() == ("booking", "listing__user")
