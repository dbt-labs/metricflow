...
def test_union() -> None:  # noqa: D103
    ...
    assert MutableDunderNameTrie.union_exclusive([left_trie, right_trie]).dunder_names() == (
        "listing",
        "listing__country",
    )
...
def test_intersection() -> None:  # noqa: D103
    ...
    assert MutableDunderNameTrie.intersection_merge([left_trie, right_trie]).dunder_names() == (
        "booking",
        "listing__user",
    )
