...
def test_time_dimension_subgraph(  # noqa: D103
    ...
    measure_attribute_subgraph_generator = MeasureAttributeSubgraphGenerator(
        SubgraphGeneratorArgumentSet(manifest_object_lookup=sg_00_minimal_manifest_lookup)
    )
    subgraph.update(measure_attribute_subgraph_generator.generate_subgraph())

    time_dimension_subgraph_generator = TimeDimensionSubgraphGenerator(
        SubgraphGeneratorArgumentSet(manifest_object_lookup=sg_00_minimal_manifest_lookup)
    )
    subgraph.update(time_dimension_subgraph_generator.generate_subgraph())
