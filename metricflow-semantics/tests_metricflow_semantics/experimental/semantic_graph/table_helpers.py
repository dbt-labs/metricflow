...
class TextTableComparisonHelper:
    """A helper to format text tables when used for comparisons in tests.

    The issue with comparing text tables is that if there's mismatch due to differing values in a column, the width
    of the columns can change, resulting in a diff for many lines.

    This formatter generates text tables that pad columns using whitespace so that in case of a missing row, you don't
    get diff lines for the whole table.
    """
    ...
    def _record_column_widths(self, rows: Sequence[Mapping[str, str]]) -> None:
        """For each row, update the maximum width of the column name / value."""
        for row in rows:
    ...
    def add_left_rows(self, rows: Sequence[Mapping[str, str]]) -> None:  # noqa: D102
        self._record_column_widths(rows)
    ...
    def assert_tables_equal(self, log_result_table: bool) -> None:
        """Assert that the left and right tables match."""
        left_table = self.format_left_table()
        right_table = self.format_right_table()

        if left_table == right_table:
            additional_format_kwargs = {"table_text": left_table} if log_result_table else {}
