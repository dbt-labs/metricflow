...
from metricflow_semantics.experimental.mf_graph.formatting.graphviz_html import GraphvizHtmlAlignment, GraphvizHtmlText
...
class GraphvizHtmlTableBuilder:
    ...
    def __init__(self, table_cell_padding: int = 0, table_border: int = 0) -> None:
    ...
    def new_row_builder(self) -> Iterator[_GraphvizHtmlTableRowBuilder]:
        ...
        with self._indented_section():
            for line in row_builder.build():
                self._add_line(line)

    def add_horizontal_rule(self) -> Self:
        self._add_line("<HR/>")
        return self

    def build(self) -> AnyLengthTuple[str]:
        if self._build_result is not None:
...
class _GraphvizHtmlTableRowBuilder:
    """For use with `GraphvizHtmlTableBuilder` - builds a row in the table."""
