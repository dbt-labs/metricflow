...
# class SemanticGraph2:
#
#     def __init__(self, nodes: Iterable[SemanticGraphNode], edges: Iterable[SemanticGraphEdge]) -> None:
#         self._edge_by_head_node: Dict[SemanticGraphNode, S]
#         self._edge_by_tail_node = {edge.tail_node: edge for edge in edges}
#         raise NotImplementedError
#
#
#     def edges_with_tail_node(self, tail_node: SemanticGraphNode) -> Iterable[SemanticGraphEdge]
#         for edge in
...
class SemanticGraph:
    ...
    def get_edges_to_node(self, node: SemanticGraphNode) -> Sequence[SemanticGraphEdge]:
        matching_edges = []
        for edge in self.edges:
            if edge.head_node == node:
                matching_edges.append(edge)
        return matching_edges

    def get_edges_from_node(self, node: SemanticGraphNode) -> Sequence[SemanticGraphEdge]:
        matching_edges = []
        for edge in self.edges:
            if edge.tail_node == node:
                matching_edges.append(edge)
        return matching_edges
