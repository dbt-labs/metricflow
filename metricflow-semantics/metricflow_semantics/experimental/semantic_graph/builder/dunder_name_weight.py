...
class DunderNameWeightFunction(WeightFunction[SemanticGraphNode, SemanticGraphEdge, AttributeRecipeWriterPath]):
    ...
    def __init__(  # noqa: D107
        ...
        property_deny_set: Optional[Set[LinkableElementProperty]] = None,
    ) -> None:
    ...
    def incremental_weight(
        ...
        property_deny_set = self._property_deny_set
        ...
        next_recipe_element_type = next_attribute_recipe.element_type

        model_ids_in_current_path = current_recipe.models_in_join
        if next_recipe_element_type is not LinkableElementType.METRIC:
            if len(next_attribute_recipe.models_in_join) != len(set(next_attribute_recipe.models_in_join)):
                if self._verbose_debug_logs:
                    logger.debug(
                        LazyFormat(
        ...
        if next_recipe_element_type is not None:
