...
class GroupByMetricTrieResolver(DunderNameTrieResolver):
    ...
    def _resolve_trie(
        ...
        metric_property = LinkableElementProperty.METRIC
        if element_filter is not None:
            metric_names_allow_set = element_filter.element_names
            if metric_property in element_filter.without_any_of or metric_property not in element_filter.with_any_of:
                logger.debug(
                    LazyFormat(
        ...
        entity_key_trie: DunderNameTrie = MutableDunderNameTrie.intersection(
            tuple(
...
class _MetricNameToEntityKeyTrieGenerator:
    ...
    def _get_entity_key_trie_for_metric_node(
        ...
        dunder_name_trie = MutableDunderNameTrie.intersection(key_query_sets_to_intersect)
    ...
    def generate(self) -> Mapping[str, MutableDunderNameTrie]:
        ...
        for base_metric_node in base_metric_nodes:
            ...
            dunder_name_trie = MutableDunderNameTrie.intersection(entity_key_trie_intersection_list)
