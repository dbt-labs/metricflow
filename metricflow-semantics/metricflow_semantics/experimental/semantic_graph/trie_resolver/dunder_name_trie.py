...
from typing import Optional, Self, Sequence, TypeVar, override
...
DunderNameTrieT = TypeVar("DunderNameTrieT", bound="DunderNameTrie")


class DunderNameTrie(ABC):
    """A read-only interface for a trie-based data structure that stores dunder names and properties.
    ...
    @classmethod
    @abstractmethod
    def union(cls, trie_sequence: Sequence[Self]) -> Self:
        """Union a sequence of tries.

        In a union operation, non-unique names are discarded. This is to represent ambiguous join paths which are not
    ...
    @classmethod
    @abstractmethod
    def intersection(cls, trie_sequence: Sequence[Self]) -> Self:
        """Intersect a sequence of tries.

        In an intersection operation, names that do not exist in all tries are discarded. This is to represent the need
        for a group-by name to be available for all metrics that are inputs to derived metric.
        """
        raise NotImplementedError
    ...
    def name_element_to_descriptor(self) -> Mapping[str, DunderNameDescriptor]:
        """Return a mapping for the name elements in this trie to the descriptor.

        For example, this would return `{"listing": Descriptor(...)}` for a trie that stored `listing__country`. See
        `name_element_to_next_trie` for retrieval of the `country` element.
        """
        raise NotImplementedError
    ...
    def next_name_element_to_trie(self) -> Mapping[str, MutableDunderNameTrie]:
        ...
        For example, a trie that represents `listing__country` would return `{"country": DunderNameTrie(...)}`.
        """
        raise NotImplementedError

    def mutable_copy(self) -> MutableDunderNameTrie:
        """Return a mutable copy of this trie."""
...
class MutableDunderNameTrie(DunderNameTrie):
    ...
    @classmethod
    @override
    def union(cls, trie_sequence: Sequence[DunderNameTrie]) -> MutableDunderNameTrie:
        ...
        for name_element, next_tries in next_name_element_to_tries_for_union.items():
            new_name_element_to_next_trie[name_element] = MutableDunderNameTrie.union(next_tries)
    ...
    @classmethod
    @override
    def intersection(cls, trie_sequence: Sequence[DunderNameTrie]) -> MutableDunderNameTrie:
        if len(trie_sequence) == 0:
            return MutableDunderNameTrie()
        elif len(trie_sequence) == 1:
            return trie_sequence[0].mutable_copy()
        ...
        for trie in other_tries:
            for name_element in intersected_name_elements_for_descriptors:
                new_element_name_to_descriptor[name_element] = new_element_name_to_descriptor[name_element].union(
        ...
        new_name_element_to_next_trie = {
            name_element: MutableDunderNameTrie.intersection(next_tries)
    ...
    def update_derived_from_model_ids(self, derived_from_model_ids: AnyLengthTuple[SemanticModelId]) -> None:
        ...
        self.name_element_to_descriptor_dict.update(
            {
                name_element: descriptor.union_derived_from_model_ids(derived_from_model_ids)
    ...
    def add_name_items(self, items: Sequence[tuple[IndexedDunderName, DunderNameDescriptor]]) -> None:
        """Build out this trie by adding individual names."""
        index_to_ambiguous_name_elements: dict[int, set[str]] = defaultdict(set)
        for indexed_dunder_name, descriptor in items:
            ...
            # Traverse the trie to find the leaf trie.
            current_trie = self
            for name_element_index in range(name_element_count):
                ...
                if name_element_index == name_element_count - 1:
                    ...
                    if current_name_element in index_to_ambiguous_name_elements[name_element_index]:
                        continue
                    ...
                    index_to_ambiguous_name_elements[name_element_index].add(current_name_element)
                    continue
    ...
    def add_name_item(
        ...
        In general, items should not be added one at a time due to the overhead in method calls.
        """
        self.add_name_items([(indexed_dunder_name, descriptor)])

    @override
    def name_items(self, max_length: Optional[int] = None) -> Sequence[tuple[IndexedDunderName, DunderNameDescriptor]]:
        item_collector: list[tuple[IndexedDunderName, DunderNameDescriptor]] = []
        self._collect_name_items(
            name_element_prefix=(), item_collector=item_collector, current_length=1, max_length=max_length
        )
    ...
    def _collect_name_items(
        ...
        """Helper method to collect all names represented in the trie."""
        ...
        for name_element, next_node in self.name_element_to_next_trie_dict.items():
            next_node._collect_name_items(
                name_element_prefix + (name_element,), item_collector, current_length + 1, max_length
            )
