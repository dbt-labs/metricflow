...
class DunderNameTrie(ABC):
    ...
    @classmethod
    @abstractmethod
    def union_exclusive(cls, trie_sequence: Sequence[Self]) -> Self:
    ...
    @classmethod
    @abstractmethod
    def intersection_merge(cls, trie_sequence: Sequence[Self]) -> Self:
        """Intersect names from a sequence of tries, and merge the associated descriptors.
...
class MutableDunderNameTrie(DunderNameTrie):
    ...
    @classmethod
    @override
    def union_exclusive(cls, trie_sequence: Sequence[DunderNameTrie]) -> MutableDunderNameTrie:
        ...
        for name_element, next_tries in next_name_element_to_tries_for_union.items():
            new_name_element_to_next_trie[name_element] = MutableDunderNameTrie.union_exclusive(next_tries)
    ...
    @classmethod
    @override
    def intersection_merge(cls, trie_sequence: Sequence[DunderNameTrie]) -> MutableDunderNameTrie:
        if len(trie_sequence) == 0:
            return MutableDunderNameTrie()
        elif len(trie_sequence) == 1:
            return trie_sequence[0].mutable_copy()
        ...
        new_name_element_to_next_trie = {
            name_element: MutableDunderNameTrie.intersection_merge(next_tries)
