...
class EntityKeyTrieResolver:
    """A resolver to figure out the possible group-by items that are entity-keys.
     ...
     This is also known as the available entities in a "query for an entity".
    ...
    The entity-key trie is needed to resolve the available group-by metrics for a given metric. Group-by metrics
    are modeled as a join using a subquery where the group-by item is the entity key. e.g. for the query (`bookings` by
    `listing__views` ), the query (`bookings` by `listing`) is joined with the query (`views` by `listing`).
    """
