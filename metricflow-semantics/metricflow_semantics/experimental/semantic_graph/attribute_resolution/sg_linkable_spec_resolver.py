...
class SemanticGraphLinkableSpecResolver(LinkableSpecResolver):
    ...
    def __init__(
        ...
        self._simple_one_model_resolver = SimpleTrieResolver(semantic_graph, path_finder, max_path_model_count=1)
        self._group_by_metric_resolver = GroupByMetricTrieResolver(semantic_graph, path_finder)
        self._local_model_nodes = self._semantic_graph.nodes_with_labels(
            LocalModelLabel.get_instance(), MetricTimeLabel.get_instance()
        )
        ...
        self._result_cache_for_distinct_values: ResultCache[tuple[()], BaseLinkableElementSet] = ResultCache()
    ...
    def get_linkable_elements_for_distinct_values_query(
        ...
        for local_model_node in self._local_model_nodes:
            ...
            local_model_trie = self._simple_one_model_resolver.resolve_trie(
                source_nodes, element_filter
            ).dunder_name_trie
            group_by_metric_trie = self._group_by_metric_resolver.resolve_trie(
                source_nodes, element_filter
        ...
        return self._result_cache_for_distinct_values.set_and_get(
            cache_key, AnnotatedSpecLinkableElementSet.create_from_trie(*tries_to_union)
        )
