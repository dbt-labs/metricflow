...
class AttributeRecipeWriterWeightFunction(
    ...
    * Querying a time dimension at a time grain that's smaller than the one configured in the semantic model.
    ...
    def incremental_weight(
        ...
        if element_filter and self._filter_denies_recipe(element_filter, next_node, next_recipe):
            if self._verbose_debug_logs:
                logger.debug(
                    LazyFormat(
                        "Blocking edge due to the element filter.",
    ...
    def _filter_denies_recipe(
        ...
        next_node_is_in_time_cluster = self._time_cluster_label in next_node.labels
        # We don't want to block the path from time dimension -> time entity, so check the time cluster label.
        # The time-entity node label could be used as well.
        if not next_node_is_in_time_cluster:
