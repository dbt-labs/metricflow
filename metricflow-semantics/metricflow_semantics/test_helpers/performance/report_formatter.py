...
FormatterOutputT = TypeVar("FormatterOutputT")
...
class TableTextFormatter(SessionReportTextFormatter):
    ...
    def _justify_and_pad_column(
        ...
        """
        max_column_length = max(len(column_name), max(len(row[column_name]) for row in rows))
        for row in rows:
            previous_value = row[column_name]
            row[column_name] = f"{previous_value:{pad_character}{" < " if left_justify else " > "}{max_column_length}}"
