...
def parse_manifest_from_dbt_generated_manifest(manifest_json_string: str) -> PydanticSemanticManifest:
    ...
    rules = (
        # Primary
        (LowerCaseNamesRule(),),
        # Secondary - broken out into groups because we run DedupeMetricInputSimpleMetricInputsRule in the middle.
        (
            *rule_set.legacy_measure_update_rules,
            # These individual rules come from rule_set.convert_legacy_simple_metric_inputs_to_metrics_rules, but
            # dsi requØires AddInputMetricSimpleMetricInputsRule, and metricflow requires that we do NOT run that rule
            # as it is incompatible with a parser like dbt-core that pre-populates input simple-metric inputs.
