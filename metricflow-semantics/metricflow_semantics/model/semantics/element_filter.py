...
class LinkableElementFilter(Mergeable):
    ...
    def allow(self, element_name: str, element_properties: Iterable[LinkableElementProperty]) -> bool:
        ...
        if allowed_element_name_set is not None and element_name not in allowed_element_name_set:
            return False
        allowed_property_set = self.with_any_of
        if allowed_property_set is not None and len(allowed_property_set.intersection(element_properties)) == 0:
            return False
        denied_property_set = self.without_any_of
        if denied_property_set is not None and len(denied_property_set.intersection(element_properties)) > 0:
            return False
