...
from dbt_semantic_interfaces.call_parameter_sets import EntityCallParameterSet, MetricCallParameterSet
...
class WhereFilterMetric(ProtocolHint[QueryInterfaceMetric]):
    """An metric that is passed in through the where filter parameter."""
...
class WhereFilterMetricFactory(ProtocolHint[QueryInterfaceMetricFactory]):
    ...
    Each call to `create` adds an MetricSpec to metric_specs.
    """
    ...
    def create(self, metric_name: str, entity_path: Sequence[str] = ()) -> WhereFilterMetric:
        """Create a WhereFilterMetric."""
        structured_name = DunderedNameFormatter.parse_name(metric_name.lower())
