...
ON
  (
    ...
    (
      subq_13.metric_time__day >= subq_18.window_start__extract_week
    ) AND (
      (
        subq_13.metric_time__day < subq_18.window_end__extract_dayofyear
      ) OR (
        subq_18.window_end__extract_dayofyear IS NULL
      )
    )
  )
GROUP BY
