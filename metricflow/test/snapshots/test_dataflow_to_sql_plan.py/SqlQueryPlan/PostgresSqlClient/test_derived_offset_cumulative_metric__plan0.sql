  ...
  FROM (
    ...
    FROM (
      ...
      FROM (
        ...
        SELECT
          DATE_TRUNC('day', subq_5.metric_time) AS metric_time
          , subq_4.ds AS ds
        ...
        INNER JOIN (
          ...
          ON
            (
              ...
              subq_1.metric_time > subq_2.metric_time - MAKE_INTERVAL(days => 2)
            )
        ) subq_4
        ON
          subq_5.metric_time - MAKE_INTERVAL(days => 2) = subq_4.metric_time
      ) subq_7
    ) subq_8
    GROUP BY
      subq_8.metric_time
