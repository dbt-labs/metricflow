  ...
  FROM (
    ...
    FROM (
      ...
      FROM (
        ...
        WHERE subq_4.ds BETWEEN CAST('2020-01-01' AS DATETIME) AND CAST('2020-01-01' AS DATETIME)
        GROUP BY
          metric_time
      ) subq_3
      INNER JOIN (
      ...
      ON
        (
          subq_2.metric_time <= subq_3.metric_time
        ) AND (
          subq_2.metric_time > DATE_SUB(CAST(subq_3.metric_time AS DATETIME), INTERVAL 2 month)
        )
    ) subq_5
  ) subq_6
  GROUP BY
