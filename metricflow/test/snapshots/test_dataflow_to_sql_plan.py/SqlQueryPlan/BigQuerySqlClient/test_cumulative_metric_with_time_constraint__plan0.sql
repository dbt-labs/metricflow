  ...
  FROM (
    ...
    FROM (
      ...
      FROM (
        -- Date Spine
        SELECT
          DATE_TRUNC(subq_4.ds, month) AS ds__month
        FROM ***************************.mf_time_spine subq_4
        WHERE (
          subq_4.ds >= CAST('2020-01-01' AS DATETIME)
        ) AND (
          subq_4.ds <= CAST('2020-01-01' AS DATETIME)
        )
        GROUP BY
          DATE_TRUNC(subq_4.ds, month)
      ) subq_3
      INNER JOIN (
        -- Pass Only Elements:
        ...
        FROM (
          ...
          WHERE (
            subq_0.ds >= CAST('2019-12-01' AS DATETIME)
          ) AND (
            subq_0.ds <= CAST('2020-01-01' AS DATETIME)
          )
        ) subq_1
      ) subq_2
      ON
        (
          ...
          subq_2.ds__month > DATE_SUB(CAST(subq_3.ds__month AS DATETIME), INTERVAL 2 month)
        )
    ) subq_5
    WHERE (
      subq_5.ds__month >= CAST('2020-01-01' AS DATETIME)
    ) AND (
      subq_5.ds__month <= CAST('2020-01-01' AS DATETIME)
    )
  ) subq_6
