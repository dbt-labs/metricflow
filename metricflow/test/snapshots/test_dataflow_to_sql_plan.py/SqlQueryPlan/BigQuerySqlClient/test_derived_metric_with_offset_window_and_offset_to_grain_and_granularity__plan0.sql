    ...
    FROM (
      ...
      FROM (
        ...
        FROM (
          ...
          SELECT
            DATE_TRUNC(subq_2.metric_time, isoyear) AS metric_time__year
            , subq_1.ds AS ds
          ...
          ON
            DATE_TRUNC(subq_2.metric_time, month) = subq_1.metric_time
        ) subq_4
      ) subq_5
      GROUP BY
        metric_time__year
    ...
    FROM (
      ...
      FROM (
        ...
        FROM (
          -- Join to Time Spine Dataset
          SELECT
            DATE_TRUNC(subq_10.metric_time, isoyear) AS metric_time__year
            , subq_9.ds AS ds
          ...
          ON
            DATE_SUB(CAST(subq_10.metric_time AS DATETIME), INTERVAL 1 month) = subq_9.metric_time
        ) subq_12
      ) subq_13
      GROUP BY
        metric_time__year
