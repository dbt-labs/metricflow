...
FROM (
  ...
  FROM (
    ...
    ON
      DATE_TRUNC(subq_5.metric_time__day, month) = subq_4.metric_time__day
  ) subq_7
  INNER JOIN (
    -- Join to Time Spine Dataset
    SELECT
    ...
    ON
      DATE_SUB(CAST(subq_13.metric_time__day AS DATETIME), INTERVAL 1 month) = subq_12.metric_time__day
  ) subq_15
  ON
    (
