...
ON
  (
    ...
    (
      subq_13.metric_time__day >= subq_18.lux_listing__window_start__extract_dayofweek
    ) AND (
      (
        subq_13.metric_time__day < subq_18.lux_listing__window_end__extract_month
      ) OR (
        subq_18.lux_listing__window_end__extract_month IS NULL
      )
    )
  )
GROUP BY
