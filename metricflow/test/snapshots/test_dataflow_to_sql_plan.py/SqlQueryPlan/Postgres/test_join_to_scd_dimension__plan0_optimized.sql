...
FROM (
  ...
  ON
    (
      ...
      (
        subq_12.metric_time__day >= EXTRACT(DAYOFYEAR FROM listings_src_10017.active_from)
      ) AND (
        (
          subq_12.metric_time__day < EXTRACT(QUARTER FROM listings_src_10017.active_to)
        ) OR (
          EXTRACT(QUARTER FROM listings_src_10017.active_to) IS NULL
        )
      )
    )
) subq_16
