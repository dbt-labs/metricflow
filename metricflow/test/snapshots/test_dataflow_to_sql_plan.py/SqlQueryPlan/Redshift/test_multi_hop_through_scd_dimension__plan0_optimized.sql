...
ON
  (
    ...
    (
      subq_13.metric_time__day >= subq_18.window_start__extract_year
    ) AND (
      (
        subq_13.metric_time__day < subq_18.window_end__extract_year
      ) OR (
        subq_18.window_end__extract_year IS NULL
      )
    )
  )
GROUP BY
