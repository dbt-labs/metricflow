...
FROM (
  ...
  LEFT OUTER JOIN (
    ...
    SELECT
      ...
      , COUNT(DISTINCT bookings_source_src_28001.guest_id) AS bookers
    FROM ***************************.mf_time_spine subq_13
    INNER JOIN
      ***************************.fct_bookings bookings_source_src_28001
    ON
      (
        DATE_TRUNC(bookings_source_src_28001.ds, day) <= subq_13.ds
      ) AND (
        DATE_TRUNC(bookings_source_src_28001.ds, day) > DATE_SUB(CAST(subq_13.ds AS DATETIME), INTERVAL 2 day)
      )
    GROUP BY
      metric_time__day
  ) subq_16
