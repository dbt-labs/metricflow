  ...
  FULL OUTER JOIN (
    ...
    WHERE (
      (
        (
          (
            DATE_TRUNC('week', subq_22.ds) = subq_22.ds
          ) OR (
            DATE_TRUNC('month', subq_22.ds) = subq_22.ds
          )
        ) OR (
          DATE_TRUNC('quarter', subq_22.ds) = subq_22.ds
        )
      ) OR (
        DATE_TRUNC('year', subq_22.ds) = subq_22.ds
      )
    ) OR (
      DATE_TRUNC('day', subq_22.ds) = subq_22.ds
    )
    GROUP BY
      subq_22.ds
  ) subq_26
