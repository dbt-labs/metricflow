...
def test_validity_window_configuration() -> None:
    """Tests to ensure a entity with a properly configured validity window passes validation"""
...
def test_multiple_validity_windows_are_invalid() -> None:
    """Tests validation asserting that no more than 1 validity window can exist in a entity"""
    yaml_contents = textwrap.dedent(
        """\
        entity:
          name: scd_entity
...
def test_measures_are_prevented() -> None:
    """Tests validation asserting that measures are not allowed in a entity with validity windows

    This block is temporary while we sort out the proper syntax for defining a measure in SCD-style entities
...
def test_validity_window_must_have_a_natural_key() -> None:
    """Tests validation asserting that entities with validity windows use an identifier with type NATURAL"""
...
def test_validity_window_does_not_use_primary_key() -> None:
    """Tests validation asserting that entities with validity windows do not use primary keys

    This is useful because we currently do not support joins against SCD-style entities without using the
