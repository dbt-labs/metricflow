...
from metricflow.naming.linkable_spec_name import GroupByInputSpec, MetricInputSpec, OrderByInputSpec
...
def test_query_parser_v2(time_spine_source: TimeSpineSource) -> None:  # noqa: D
    ...
    query_spec = query_parser.parse_and_validate_query_v2(
        metric_input_specs=[MetricInputSpec("bookings")],
        group_by_input_specs=[GroupByInputSpec("is_instant"), GroupByInputSpec("listing"), GroupByInputSpec(MTD)],
        order_specs=[OrderByInputSpec(MTD), OrderByInputSpec("bookings", descending=True)],
    )
...
def test_order_by_granularity_conversion(time_spine_source: TimeSpineSource) -> None:
    ...
    query_spec = query_parser.parse_and_validate_query_v2(
        metric_input_specs=[MetricInputSpec("bookings"), MetricInputSpec("revenue")],
        group_by_input_specs=[GroupByInputSpec(MTD)],
        order_specs=[OrderByInputSpec(MTD, descending=True)],
    )
...
def test_order_by_granularity_no_conversion(time_spine_source: TimeSpineSource) -> None:  # noqa: D
    ...
    query_spec = query_parser.parse_and_validate_query_v2(
        metric_input_specs=[MetricInputSpec("bookings")],
        group_by_input_specs=[GroupByInputSpec(MTD)],
        order_specs=[OrderByInputSpec(MTD)],
    )
...
def test_column_override(time_spine_source: TimeSpineSource) -> None:
    ...
    query_spec = query_parser.parse_and_validate_query_v2(
        metric_input_specs=[MetricInputSpec("revenue")],
        group_by_input_specs=[GroupByInputSpec(MTD)],
        time_constraint_start=as_datetime("2020-01-15"),
        time_constraint_end=as_datetime("2020-02-15"),
    )
...
def test_parse_and_validate_where_constraint_dims(time_spine_source: TimeSpineSource) -> None:
    ...
    with pytest.raises(UnableToSatisfyQueryError):
        query_parser.parse_and_validate_query_v2(
            metric_input_specs=[MetricInputSpec("bookings")],
            group_by_input_specs=[GroupByInputSpec(MTD)],
            time_constraint_start=as_datetime("2020-01-15"),
            time_constraint_end=as_datetime("2020-02-15"),
...
def test_parse_and_validate_metric_constraint_dims(time_spine_source: TimeSpineSource) -> None:
    ...
    with pytest.raises(UnableToSatisfyQueryError):
        query_parser.parse_and_validate_query_v2(
            metric_input_specs=[MetricInputSpec("metric_with_invalid_constraint")],
            group_by_input_specs=[GroupByInputSpec(MTD)],
            time_constraint_start=as_datetime("2020-01-15"),
            time_constraint_end=as_datetime("2020-02-15"),
        )
...
def test_derived_metric_query_parsing(time_spine_source: TimeSpineSource) -> None:
    ...
    with pytest.raises(UnableToSatisfyQueryError):
        query_parser.parse_and_validate_query_v2(
            metric_input_specs=[MetricInputSpec("revenue_sub_10")],
            group_by_input_specs=[],
        )
    ...
    with pytest.raises(UnableToSatisfyQueryError):
        query_parser.parse_and_validate_query_v2(
            metric_input_specs=[MetricInputSpec("revenue_sub_10")],
            group_by_input_specs=[GroupByInputSpec("country")],
        )
...
def test_derived_metric_with_offset_parsing(time_spine_source: TimeSpineSource) -> None:
    ...
    with pytest.raises(UnableToSatisfyQueryError):
        query_parser.parse_and_validate_query_v2(
            metric_input_specs=[MetricInputSpec("revenue_growth_2_weeks")],
            group_by_input_specs=[],
        )
