...
class DuckDbSqlExpressionRenderer(DefaultSqlExpressionRenderer):
    ...
    def visit_percentile_expr(self, node: SqlPercentileExpression) -> SqlExpressionRenderResult:
        ...
        if node.percentile_args.function_type is SqlPercentileFunctionType.CONTINUOUS:
            ...
            raise RuntimeError(
                "Approximate discrete percentile aggregate not supported for DuckDB. Set use_discrete_percentile and/or use_approximate_percentile to false in all measures."
            )
        else:
