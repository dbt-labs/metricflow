...
class FireboltSqlExpressionRenderer(DefaultSqlExpressionRenderer):
    """Expression renderer for the Firebolt engine.

    Firebolt is PostgreSQL-compatible, so this renderer follows the same patterns as Postgres.
    """

    sql_engine = SqlEngine.FIREBOLT

    @property
    @override
    def double_data_type(self) -> str:
        """Custom double data type for the Firebolt engine."""
    ...
    def visit_subtract_time_interval_expr(self, node: SqlSubtractTimeIntervalExpression) -> SqlExpressionRenderResult:
        """Render time delta operations for Firebolt, which needs custom support for quarterly granularity."""
    ...
    def visit_percentile_expr(self, node: SqlPercentileExpression) -> SqlExpressionRenderResult:
        """Render a percentile expression for Firebolt."""
        ...
        if node.percentile_args.function_type is SqlPercentileFunctionType.CONTINUOUS:
            ...
            raise UnsupportedEngineFeatureError(
                "Approximate continuous percentile aggregate not supported for Firebolt. Set "
            ...
            raise UnsupportedEngineFeatureError(
                "Approximate discrete percentile aggregate not supported for Firebolt. Set "
                + "use_approximate_percentile to false in all percentile simple-metrics."
            )
        else:
...
class FireboltSqlPlanRenderer(DefaultSqlPlanRenderer):
    """Plan renderer for the Firebolt engine."""

    EXPR_RENDERER = FireboltSqlExpressionRenderer()
