...
class ValidLinkableSpecResolver:
    ...
    def _get_semantic_models_joinable_to_entity(
        ...
        """Find all semantic models that contain this entity, split into those with valid vs. fan-out joins."""
    ...
    def _get_single_hop_join_paths(
        self,
        semantic_model: SemanticModel,
    ) -> Tuple[Sequence[SemanticModelJoinPath], Sequence[SemanticModelJoinPath]]:
        """Get all valid & fan-out join paths to join other semantic models to this one."""
        valid_join_paths = []
        fan_out_join_paths = []
        for entity in semantic_model.entities:
            (
                semantic_models_with_valid_join,
                semantic_models_with_fan_out_join,
            ) = self._get_semantic_models_joinable_to_entity(
            ...
            fan_out_join_paths += [
                SemanticModelJoinPath.from_single_element(joinable_semantic_model.reference, entity.reference)
                for joinable_semantic_model in semantic_models_with_fan_out_join
                if joinable_semantic_model.name != semantic_model.name
            ]
        return valid_join_paths, fan_out_join_paths
    ...
    def _get_joined_elements(self, measure_semantic_model: SemanticModel) -> LinkableElementSet:
        ...
        single_hop_properties = frozenset({LinkableElementProperties.JOINED})

        valid_join_paths, fan_out_join_paths = self._get_single_hop_join_paths(measure_semantic_model)
        basic_join_elements = [
            self.create_linkable_element_set_from_join_path(join_path=join_path, with_properties=single_hop_properties)
            for join_path in valid_join_paths
        ]

        # Since LinkableMetrics will be aggregated on the right side, we can allow what would normally be fan-out joins.
        fan_out_join_elements = LinkableElementSet(
            path_key_to_linkable_metrics={
                linkable_metric.path_key: (linkable_metric,)
                for fan_out_join_path in fan_out_join_paths
                for linkable_metric in self._create_linkable_metrics_from_join_path(
                    join_path=fan_out_join_path, with_properties=single_hop_properties
                )
            },
        )
        single_hop_elements = LinkableElementSet.merge_by_path_key(basic_join_elements + [fan_out_join_elements])

        # TODO: determine if multi-hop fan-out join is acceptable for LinkableMetrics. Currently not enabled.
