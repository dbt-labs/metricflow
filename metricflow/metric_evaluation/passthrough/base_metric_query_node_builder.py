...
class BaseMetricQueryNodeBuilder:
    ...
    In general, this creates the corresponding `MetricQueryNode` subclass instance appropriate for the metric type in
    ...
    * The time-offsets are the same.
    ...
    Otherwise, separate nodes should be created. Since the node will eventually be converted to a SQL query that reads
    from the semantic model, these requirement reflects the associated limitations (e.g. a `WHERE` clause applies to
    the source, so you need different queries to apply different filters).
    ...
    def build_nodes(self, query_elements: Iterable[MetricQueryElement]) -> Sequence[MetricQueryNode]:  # noqa: D102
        ...
        for query_element in query_elements:
            ...
            group_key = BaseMetricsQueryElementGroupKey(
                model_id=model_id,
                metric_type=metric_type,
                non_simple_metric_spec=metric_spec if metric_type is not MetricType.SIMPLE else None,
                aliased_metric=query_element.metric_spec if query_element.metric_spec.alias is not None else None,
        ...
        for group_key, metric_specs in group_key_to_metric_specs.items():
            ...
            if metric_type is MetricType.SIMPLE:
                ...
                if len(metric_specs) != 1:
                    raise MetricFlowInternalError(
                        LazyFormat("Expected exactly 1 metric spec", metric_specs=metric_specs)
                    )
                nodes.append(
                    CumulativeMetricQueryNode.create(
                        metric_spec=metric_specs[0],
                        query_properties=query_properties,
                    )
                )
...
class BaseMetricsQueryElementGroupKey:
    """A key to use for grouping query elements for base metrics."""

    # Not needed as `non_simple_metric_spec` will prevent grouping of simple metrics with non-simple metrics, but
    # saves a lookup.
    metric_type: MetricType
    # Ensures that metrics with an alias are put in separate group.
    aliased_metric: Optional[MetricSpec]
    # `Ensures that only simple metrics with the same model can be put into the same group.
    ...
    # Ensures filters and other query properties are the same in each group.
