...
from scripts.release_tool.github_client import GitHubClient, GitHubMergeMethod
...
class ReleaseStep6State(BaseModel):
    ...
    # Tag created for the dbt-metricflow release.
    dbt_metricflow_release_tag_name: str | None = None
    # Merge commit SHA for the step 4 PR that creates the dbt-metricflow release.
    dbt_metricflow_release_merge_commit_sha: str
    # Merge commit SHA for the step 5 PR that changes the dbt-metricflow package version to a dev one.
    dbt_metricflow_dev_version_commit_sha: str
...
class ReleaseStep6Runner:
    ...
    * Creating or updating a lightweight ``dbt-metricflow/v$VERSION`` tag
    ...
    def run(self) -> ReleaseStep6State:
        ...
        tag_name = f"{ReleaseHelper.DBT_METRICFLOW_RELEASE_TAG_PREFIX}{dbt_metricflow_version}"
        ...
        return ReleaseStep6State(
            ...
            dbt_metricflow_dev_version_commit_sha=step_5_merge_sha,
        )
