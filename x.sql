...
-- TODO: get CTE working somehow. Might need a separate DFP node.
with cte as (
    SELECT
        date_day,
        fiscal_quarter
...
INNER JOIN (
    ...
    -- OffsetByCustomGranularityNode
        select
            fiscal_quarter
            , case
                when dateadd(day, ds__day__row_number - 1, fiscal_quarter_start__offset_by_1) <= fiscal_quarter_end__offset_by_1
                    then dateadd(day, ds__day__row_number - 1, fiscal_quarter_start__offset_by_1)
